...
draws <- 
  rnorm(number_of_draws, mean = 0, sd = 1)

estimator_one <- sum(draws) / number_of_draws
estimator_two <- (sum(draws) / number_of_draws) + 0.5
...
# estimand <- 
#   tibble(
#     draws = rnorm(110, mean = 0, sd = 1),
#     number = c(rep(x = 10, times = 10), rep(x = 100, times = 100))
#   ) |>
#   group_by(number) |> 
#   summarise(
#     estimator_one = sum(draws) / number,
#     estimator_two = (sum(draws) / number) + 0.5
# )
#   
# 
# draws <- 
...
Typos and other grammatical mistakes affect the credibility of claims. If the reader cannot trust us to use a spell-checker, then why should they trust us to use logistic regression? Microsoft Word and Google Docs are useful here for their spell-checkers: copy/paste from the Quarto document, look for the red and green lines, and fix them in the Quarto document.
