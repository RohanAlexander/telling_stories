...
- Core `tidyverse` [@citetidyverse]
  ...
  - `tidyr` [@citetidyr]
    - `pivot_longer()`
    - `pivot_wider()`
- Outer `tidyverse` [@citetidyverse] (need to be loaded separately e.g. `library('haven')`)
  - `haven` [@citehaven]
    - `read_dta()`
...
![[Probability](https://xkcd.com/881/), from Randall Munroe's XKCD.](figures/probability.png){#fig-personalprobability width=90% fig-align="center"}
...
1. In the Australian politician's example, we got our data from the website GitHub using an R package, but we can get data into a workspace from a local computer in a variety of ways. One way is to use the 'upload' button in the 'Files' panel. Another is to use `readr` [@citereadr], which is part of the `tidyverse` [@citetidyverse].
...
Hadley Wickham is Chief Scientist at RStudio. After taking a PhD in Statistics from Iowa State University in 2008 he was appointed as an assistant professor at Rice University, and then became Chief Scientist at RStudio in 2013. He developed the `tidyverse` collection of packages [@citetidyverse], and has published many books including @r4ds and @advancedr. He was awarded the COPSS Presidents' Award in 2019.
:::
