...
AustralianPoliticians::get_auspol('all') |> # Imports data from GitHub
  as_tibble() |> 
...
4. *(Explore)* Assume that `ggplot2` is loaded and the dataset 'monarchs' has the column 'years'. Which of the following would result only those monarchs that lived longer than 35 years?
...
11. Say we had a dataset with two columns: 'name', and 'age'. Which verb should we use to pick just 'name' (pick one)?
    ...
    c. `tidyverse::filter()`
    d. `tidyverse::rename()`
12. Say we had loaded `AustralianPoliticians` and `tidyverse` and then run the following code: `australian_politicians <- AustralianPoliticians::get_auspol('all')`. How could we select all of the columns that end with 'Name' (pick one)? 
