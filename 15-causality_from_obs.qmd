...
- In general, these approaches need to be used with humilty and concern for weaknesses and assumptions, both those that we can test and those that we cannot.
...
In @fig-firstdag, we think that *x* causes *y*. We could build another DAG where the situation is less clear. To make the examples a little easier to follow, we will switch to thinking about a hypothetical relationship between income, education, and fertility intention (@fig-educationasconfounder).
...
In @fig-luxuryasmediator we have a similar situation where again, we may think that income causes fertility intentions. But in @fig-luxuryasmediator income also causes luxury tastes, which itself causes fertility intentions.
...
In @fig-luxuryasmediator, luxury tastes is called a "mediator" and we would not adjust for it if we were interested in the effect of income on fertility intentions. If we were to adjust for it, then some of what we are attributing to income, would be due to luxury tastes.

Finally, in @fig-residenceascollider we have yet another similar situation, where we think that income causes fertility intentions. But this time both income and fertility intentions also cause place of residence.
...
carpenter_dobkin <-
  read_dta(here::here("inputs/data/P01 Age Profile of Arrest Rates 1979-2006.dta"))
...
carpenter_dobkin_prepared |> 
  mutate(arrested_for =
           case_when(arrested_for == "assault" ~ "Assault",
  ...
  facet_wrap(facets = vars(arrested_for),
             scales = "free_y") +
  theme_minimal()
```
...
The set-up of instrumental variables is described in (@fig-taxrebateasiv), which shows education as a confounder between income and fertility intentions. A tax rebate likely only affects income, not education, and could be used as an instrumental variable.
