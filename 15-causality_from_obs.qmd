...
In @fig-dot-firstdag, we think that *x* causes *y*. We could build another DAG where the situation is less clear. To make the examples a little easier to follow, we will switch to thinking about a hypothetical relationship between income, education, and fertility intention (@fig-dot-educationasconfounder).
...
//| fig-cap: "Education is a confounder and affects the relationship between income and fertility intentions"
//| echo: true
...
In @fig-dot-educationasconfounder, we think income causes fertility intentions. But we also think that education causes fertility intentions, and that education also causes income. That relationship is a "backdoor path", and failing to adjust for education in a regression could create a spurious, or potentially overstate the extent of a, relationship between income and fertility intentions in our analysis. We may think that changes in income are causing changes in fertility intentions, but it could be that education is changing them both. That variable, in this case, education, is called a "confounder".
...
In @fig-dot-luxuryasmediator we have a similar situation where again, we may think that income causes fertility intentions. But in @fig-dot-luxuryasmediator income also causes luxury tastes, which itself causes fertility intentions.
...
//| fig-cap: "Luxury taste as a mediator between income and fertility intentions"
//| echo: true
...
In @fig-dot-luxuryasmediator, luxury tastes is called a "mediator" and we would not adjust for it if we were interested in the effect of income on fertility intentions. If we were to adjust for it, then some of what we are attributing to income, would be due to luxury tastes.

Finally, in @fig-dot-residenceascollider we have yet another similar situation, where we think that income causes fertility intentions. But this time both income and fertility intentions also cause place of residence.
...
//| fig-cap: "Residence as a collider affecting the relationship between income and fertility intentions"
//| echo: true
...
In this case, place of residence is called a "collider" and if we were to condition on it, then we would create a misleading relationship. Income influences where people live, but their fertility intentions also affect this. place of residence is then a collider because both the independent and dependent variable of interest influence it.
...
Simpson's paradox is often illustrated using real-world data from University of California, Berkeley, on graduate admissions [@bickel1975sex]. @bickel1975sex include what might be one of the greatest sub-titles ever published: "Measuring bias is harder than is usually assumed, and the evidence is sometimes contrary to expectation". More recently, as shown in its documentation, the "penguins" dataset from `palmerpenguins` [@palmerpenguins] provides an example of Simpson's paradox, using real-world data (@fig-simpsonsparadoxinpenguins).
...
library(modelsummary)
modelsummary(diff_in_diff_example_regression,
             statistic = "conf.int")
```
...
consumer_models <- list(
  ...
  "Subscriptions share" = share_of_sub,
...
carpenter_dobkin_aggravated_assault_only <-
  carpenter_dobkin_prepared |>
  filter(arrested_for == "aggravated_assault")
...
So we find that if you smoke then your health is likely to be worse than if you do not smoke.
...
The set-up of instrumental variables is described in (@fig-dot-taxrebateasiv), which shows education as a confounder between income and fertility intentions. A tax rebate likely only affects income, not education, and could be used as an instrumental variable.

<!-- ```{r} -->
<!-- #| fig-cap: "Education as a confounder for the relationship between income and fertility intentions with tax rebate as an instrumental variable" -->
