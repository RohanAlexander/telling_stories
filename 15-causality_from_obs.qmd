...
<!-- There are many opportunities to implement regression discontinuity. Elections are a common area of application for regression discontinuity because if the election is close then *prima facie* one may expect there is not much difference between the candidates. @caugheysekhon2011 showed that there is actual considerable difference between bare winners and bare losers in US House elections between 1942 an 2008, and highlight the fact that one of the advantages of regression discontinuity is the fact that the assumptions can be tests.  -->

<!-- In this example, following X, we will revisit their data. -->

<!-- We can obtain their data from [here](https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/8EYYA2). -->
