...
- Read *The Future of Data Analysis*, [@tukey1962future]
  - John Tukey, the twentieth century statistician, made a variety of contributions to statistics. Focus on Part 1 "General Considerations", which was ahead of its time about the ways in which we ought to learn something from data.
- Read *Best Practices in Data Cleaning*, [@bestpracticesindatacleaning]
  - Focus on Chapter 6 "Dealing with Missing or Incomplete Data".
- Read *R for Data Science*, [@r4ds]
  - Focus on Chapter 12 "Exploratory data analysis", which provides a written self-contained EDA worked example.
...
Interestingly, coming to thoroughly understand the data that we have often helps us understand what we do not have. We are interested in the following process:\index{exploratory data analysis!aims}
...
We want to get a quick sense of the data.\index{exploratory data analysis} The first step is to have a look at the top and bottom of it with `head()` and `tail()`, then a random selection, and finally to focus on the variables and their class with `glimpse()`. The random selection is an important aspect, and anytime you use `head()` you should also quickly consider a random selection.
...
We are then interested in understanding key summary statistics, such as the minimum, median, and maximum values for numeric variables with `summary()` from base R. 
...
Finally, it is especially important to understand the behavior of these key summary statistics at the limits.\index{exploratory data analysis!limits} In particular, one approach is to randomly remove some observations and compare what happens to them. For instance, we can randomly create five datasets that differ on the basis of which rows are removed. We can then compare the summary statistics. If any of them are especially different then we might want to look at the rows that were removed as they may contain observations with high influence\index{influential points}.
...
When observations are Missing at Random (MAR) they are missing from the dataset in a way that is related to other variables in the dataset.\index{missing data!MAR} For instance, it may be that we are interested in understanding the effect of income and gender on political participation, and so we gather information on these three variables. It might be that for some reason males are less likely to respond to a question about income. 
...
us_states_MAR <-
  us_populations |>
  mutate(population =
           if_else(state %in% highest_income_states,
                   NA_real_,
                   population))
...
The specifics of the best approach will always be bespoke to the circumstances, but in general we always want to use simulation to better understand the implications of our choices. From a data side we can choose to remove observations that are missing or input a value. (There are also options on the model side, but those are beyond the scope of this book.) These approaches have their place, but need to be used with humility and extensively communicated. The use of simulation is critical. 
...
It is important to document all aspects as we go through and note anything surprising. We are looking to create a record of the steps and assumptions that we made as we were going because these will be important when we come to modeling. In the natural sciences, a research notebook of this type can be a legal document [@nihtalk].
...
all_2021_ttc_data_no_dupes <-
  ...
  mutate(station_clean = case_when(
    str_starts(station, 
               "ST") & str_detect(station, 
                                  "WEST") ~ word(station, 1, 3),
...
It is important that we refer to this local copy of our data when we run our scripts to explore the data, rather than asking the Inside Airbnb servers for the data each time. It might be worth even commenting out this call to their servers to ensure that we do not accidentally stress their service.
...
#| fig-cap: "Distribution of prices less than $1000 for London Airbnb rentals in March 2023 shows bunching"
#| fig-subcap:
...
We can have a quick look at the results using `modelsummary()` from `modelsummary` (@tbl-modelsummarylogisticregressionsuper).
