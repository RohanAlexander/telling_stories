...
As a first example we consider US state populations\index{US states} as of 1975. This dataset is built into R with `state.x77`. Here is what the dataset looks like:
...
As a second, and more involved, example of EDA we use `opendatatoronto` [@citeSharla], introduced in @sec-fire-hose, and the `tidyverse` [@tidyverse] to obtain and explore data about the Toronto subway system\index{Toronto subway delays}. We are especially interested in getting a sense of the delays that have occurred.
