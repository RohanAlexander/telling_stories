...
We have discussed missing data a lot throughout this book, especially in @sec-farm-data. Here we return to it, because understanding missing data is something that is a substantial focus of EDA. When we find missing data---and there are always missing data of some sort or another---we want to establish what type of missingness we are dealing with. Focusing on known-missing, that is where there are observations that we can see are missing in the dataset, based on @gelmanhillvehtari2020 [p. 323] we consider three main categories of missing data: 

- Missing Completely At Random (MCAR). Here observations are missing from the dataset independent of anything else. As discussed in @sec-farm-data, when data are MCAR there is less concern, but data are rarely MCAR and even if they were it would be difficult to be convinced of this. Nonetheless we can simulate an example. For instance we can remove the population data for three randomly selected states. 
- Missing at Random (MAR). Here observations are missing from the dataset in a way that is related to the variables in the dataset. For instance, it may be that we are interested in understanding the effect of income and gender on political participation, and so we gather information on these three variables. It might be that for some reason males are less likely to respond to a question about income.
- Missing Not At Random (MNAR). Here observations are missing from the the dataset in a way that is related to unobserved variables, or the missing variable itself. For instance, it may be that respondents with a higher income, or that respondents with higher education (a variable that we did not collect), are less likely to fill in their income. 

The specifics of the best approach will always be bespoke to the circumstances, but in general we always want to use simulation to understand the implications of our choices. We can simulate an example of MCAR with the states and populations dataset.
