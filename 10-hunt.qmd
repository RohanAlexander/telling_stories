...
```{r}
#| label: bluetowhite
#| tbl-cap: "Proportion of the treatment and control group that prefer blue or white"
...
We can see the difference, which we simulated to be one, between the two groups in Figure (@fig-exampleatefig). And we can compute the average between the groups and then the difference to see also that we get back the result that we put in (@tbl-exampleatetable).
...
The specifics of this are not important, and we will have more to say in Chapter \@ref(ijalm), but they use a statistical model, @eq-oregon, to analyze the results [@finkelstein2012oregon]:
