...
Having created our dataset with each month saved to a different CSV, we can now read it in. There are a variety of ways to do this. The first step is that we need to get a list of all the CSV files in the directory. We use "glob" here to specify that we are interested only in the ".csv" files, and that could change to whatever files it is that we are interested in.
...
# Name is homage to: https://www.rand.org/pubs/monograph_reports/MR1418.html
...
number_of_draws <- 100000000

some_larger_data <-
  tibble(
    id = c(1:number_of_draws),
    state = sample(x = state.name, size = number_of_draws, replace = TRUE),
    fav_letter = sample(x = letters, size = number_of_draws, replace = TRUE),
    fav_number = runif(n = number_of_draws, min = 0, max = 100) |> round()
  )

write_csv(some_larger_data, "some_larger_data.csv")
