...
After we have put together a dataset, an important part of being responsible is storing it appropriately and enabling easy retrieval. @Wicherts2011 found that a reluctance to share data was associated with weaker evidence and more potential errors. While it is certainly possible to be especially concerned about this, and entire careers are based on the storage and retrieval of data, to a certain extent, the baseline is not onerous. If we can get our dataset off our own computer, then we are much of the way there. Further confirming that someone else can retrieve it and use it, puts us much further than most. Indeed doing this for data, models, and code represents the bronze standard according to @heil2021reproducibility.
...
We now briefly cover ways to be more efficient with data, by using SQL and parquet. We then discuss code efficiency, particularly, the need to measure and refactor code. 
