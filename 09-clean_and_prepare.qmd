...
1. Where would you save a copy of the raw data?
    a.  "inputs"
    b. "outputs"
    c. "scripts"
...
6. When dealing with ages, what is a possible class for the variable (select all that apply)?
    a. integer
    b. matrix
    c. numeric
    d. factor
7. Please consider the following cities in Germany: "Berlin", "Hamburg", "Munich", "Cologne", "Frankfurt", "Rostock". Use `testthat` to define three tests that could apply if we had a dataset with a variable "german_cities" that claimed to contain these, and only these, cities.
...
11. Using the help file for `col_types`, which of the following are options (select all that apply)?
    ...
    i.  col_datetime()
    j.  col_number()
12. With regard to @Preece1981 please discuss two ways in which final digits can be informative. Write at least a paragraph about each.
