...
When the tests of membership are passed, then finally we can change the class, and run all the tests again. We have adapted this idea from the software development approach of unit testing. Tests are crucial because they enable us to understand whether software (or in this case data) is fit for purpose [@researchsoftware]. Tests are not static things that we just write once and then forget. Instead they should update and evolve as needed.
...
Usually, special characters should be removed because they can be inconsistent between different computers and languages. This is especially the case with slash "/", backslash "\\", asterisk "*", and both single "\'" and double quotation marks "\"", none of which should almost ever be used in names. 
...
6. When dealing with ages, what are some desirable classes for the variable (select all that apply)?
    ...
    b. matrix
    c.  numeric
7. Please consider the following cities in Germany: "Berlin", "Hamburg", "Munich", "Cologne", "Frankfurt", "Rostock". Use `testthat` to define three tests that could apply if we had a dataset with a variable "german_cities" that claimed to contain these, and only these, cities. Submit a link to a GitHub Gist.
