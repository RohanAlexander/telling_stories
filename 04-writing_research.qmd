...
When answering questions, usually, the creation of a counterfactual\index{counterfactual} is crucial. A counterfactual is an if-then statement in which the "if" is false. Consider the example of Humpty Dumpty in *Through the Looking-Glass* [@throughthelookingglass].
...
A variety of authors have established rules\index{writing!rules} for writing famously including @politicsandtheenglishlanguage which were reimagined by @johnsontheeconomist. A further reimagining of rules for writing, focused on telling stories with data, could be:
