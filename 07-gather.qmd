...
Web scraping is an exciting source of data, and we will now go through some examples. But it is important to know that, in contrast to these examples, information is not usually all on one page. Web scraping quickly becomes a quite difficult art form that requires a lot of practice. For instance, it is important to distinguish between an index scrape and a contents scrape. The former is scraping to build the list of URLs that have the content you want, while the latter is actually getting the content from those URLs. An example is provided by @luscombe2022jumpstarting. If you end up doing a lot of web scraping, then `polite` [@polite] may be helpful to better optimize your workflow. And using GitHub Actions to allow for larger and slower scrapes over time.
