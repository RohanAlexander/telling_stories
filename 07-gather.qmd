...
1. Which of the following best describes an API in the context of data gathering (pick one)?
    ...
    b.  An interface provided by a server that allows clients to request data programmatically.
    c. A protocol for web browsers to render HTML content.
    d. A markup language for structuring data.
2. When using APIs for data gathering, which of the following is a common practice to authenticate requests (pick one)?
    a. Using cookies stored in the browser.
    b.  Providing an API key or token in the request.
    c. Disabling SSL verification.
    d. Modifying the hosts file on the client machine.
3. Please consider the following code, which uses `gh` to access the GitHub API (you will need to have set up GitHub on your computer, as covered in @sec-reproducible-workflows). When was the repo for `heapsofpapers` created (pick one)?
    a. 2021-02-23
    b.  2021-03-06
    c. 2021-05-25
    d. 2021-04-27
...
5. What is the main argument to `GET()` from `httr` (pick one)?
    ...
    c. "domain"
    d. "location"
6. In web scraping, what is the purpose of respecting the robots.txt file (pick one)?
    a. To ensure the scraped data is accurate.
...
14. Which function should we use if we have the following text data: "rohan_alexander" in a column called "names" and want to split it into first name and surname based on the underscore (pick one)?
    ...
    c. `spacing()`
    d. `text_to_columns()`
15. Which of the following describes Optical Character Recognition (OCR) (pick one)?
    a. A process of converting handwritten notes into typed text.
    b. A technique for parsing structured data from APIs.
    c.  A method for translating images of text into machine-encoded text.
...
18. What is the main purpose of the `purrr` package in R (pick one)?
    a. Data visualization.
    b.  Functional programming tools for iteration and mapping functions over data structures.
    c. Data manipulation.
    d. Reading and writing CSV files.
19. When performing OCR on a scanned document, what is a common issue that might affect the accuracy of text recognition (pick one)?
