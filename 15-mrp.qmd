...
One tricky aspect of MRP is ensuring consistency between the datasets. In this case, we need to do some work to make the variables consistent.
...
write_parquet(x = poststrat_data,
              sink = "outputs/data/15-poststrat_data.parquet")
```
...
poststrat_data <- 
  read_parquet(file = "outputs/data/15-poststrat_data.parquet")
```
```{r}
poststrat_data
```

We will save our cleaned post-stratification dataset as a parquet file.
...
\begin{aligned}
y_i|\pi_i & \sim \mbox{Bern}(\pi_i) \\
\mbox{logit}(\pi_i) & = \left(\alpha_0 + \alpha_{a[i]}^{\mbox{age}} + \alpha_{s[i]}^{\mbox{state}} + \alpha_{e[i]}^{\mbox{edu}} \right) \\
\alpha_0 & \sim \mbox{Normal}(0, 2.5)\\
\alpha_{a}^{\mbox{age}} & \sim \mbox{Normal}\left(0, \sigma_{\mbox{age}}\right)\mbox{ for }a = 1, 2, \dots, A\\
\alpha_{s}^{\mbox{state}} & \sim \mbox{Normal}\left(0, \sigma_{\mbox{state}}\right)\mbox{ for }s = 1, 2, \dots, S\\
...
where $y_i$ is whether a respondent supports Biden. $\alpha^\mbox{age}$, $\alpha^\mbox{state}$, and $\alpha^\mbox{edu}$ are the effect of age, state, and education, respectively. The $a[i]$, $s[i]$, and $e[i]$ refer to which age-group, state, and education level, respectively, the respondent belongs to. $A$, $S$, and $E$ are the total number of age-groups, states, and education levels, respectively. 


After reading the data in that we cleaned earlier, following @kennedygabry2020 we use `stan_glmer()` from `rstanarm` to estimate the model.
