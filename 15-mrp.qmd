...
We now train a model based on the biased survey. We explain tea preferences based on age and nationality. There is nothing that says you have to use a multilevel model, but a lot of situations will have circumstances such that it is not likely to do any worse. To be clear, this means that although we have individual-level data, there is some grouping of the individuals that we will take advantage of.

$$
\begin{aligned}
y_i|\pi_i & \sim \mbox{Bern}(\pi_i) \\
\mbox{logit}(\pi_i) & = \left(\beta_0+\beta_1 \times \mbox{age}_i + \beta_2 \times \mbox{nationality}_i\right) \\
...
where $y_i$ is the tea preference of the respondent, $\mbox{age}_i$ is the age of the respondent, and $\mbox{nationality}_i$ is the nationality of the respondent. We will estimate the model using `stan_glm()`. 
...
predicted_tea_preference <-
  tea_preference_model |>
  tidybayes::add_epred_draws(newdata = tea_poststrat_dataset,
                             value = "preference") |>
  ungroup() |>
  summarise(
