...
In this chapter we will go through various examples of EDA including US state populations, subway delays in Toronto, and Airbnb listings in London.

## 1975 US states data

As a first example we consider US state populations at at 1975. This dataset is built into R with `state.x77`.
...
We want to get a quick sense of the data. The first step is to have a look at the top and bottom of it with `head()` and `tail()`, and to focus on the variables and their class with `glimpse()`.
...
We are then interested in understanding key statistics, such as the minimum, median, and maximum values for numeric variables with `summary()` from base. We can use `skim` from `skimr` [@skimr] for simular purposes, the advantage being that it provides more information about character variables.
...
Finally, it is especially important to understand the behavior of these key summary statistics at the limits. In particular, one approach is to randomly remove some observations and compare what happens to them. For instance, we can randomly create five datasets that differ on the basis of which rows are knocked out. We can then compare the summary statistics. If any of them are especially different then we might want to look at the rows that were knocked out as they may contain observations with high-influence.

```{r}
#| echo: true
#| label: tbl-summarystatesrandom
#| tbl-cap: "Comparing the summary statistics when different states are randomly knocked out"
#| message: false
#| warning: false
...
In the case of the populations of US states, we know that larger states, such as California and New York, will have an out-sized effect on our estimate of the mean, and @tbl-summarystatesrandom supports that.


### Missing data

We have discussed missing data a lot throughout this book, and understanding missing data is something that is a substantial focus of EDA. One tempting strategy is to either remove rows that have missing data, or to infill the missing data, say with a average of the column, or using multiple imputation to fill it based on a few different options. These approaches have their place, but need to be used with humility, and extensively communicated. To understand why, we will consider the US populations data, knock out various aspects, and look at the effect on summary statistics.

We can start by knocking out the population data for three, randomly selected, states. 
...
1) Drop the observation with missing data.
2) Impute the mean for the observations with missing data.
3) Use multiple-imputation.

To drop the observations with missing data, we can use `mean()` because by default it will remove observations with missing values. 

To impute the mean, we construct a second dataset with the observations with missing data removed. We then compute the mean of the population column, and impute that into the missing values in the original dataset.

And finally, we can implement multiple-imputation with `mice()` from `mice` [@mice].
...
multiple_imputation <-
  mice(us_states_with_missing_observations,
       print = FALSE)
...
us_states_with_missing_observations_mean_imputation <-
  us_states_with_missing_observations |>
  mutate(population = if_else(is.na(us_states_with_missing_observations$population),
                              us_mean_with_missing_dropped,
                              population))
...
@tbl-imputationoptions makes it clear that none of these approaches should be blindly imposed. If imputation is the answer, it is likely that the question is wrong. Nothing can make up for missing data [@manskiwow]. The conditions under which it makes sense to impute the mean, or the prediction based on multiple imputation are not common, and even more rare is our ability to verify them. Missing data are not a nuisance to be papered over, they are our entire reason for being.
...
It is important to document all aspects as we go through and note of anything surprising. We are looking to create a record of the steps and assumptions that we made as we were going because these will be important when we come to modelling. Indeed in the natural sciences, a research notebook of this type, can be a legal document [@nihtalk]. 
...
Entire careers are spent understanding missing data, and the presence, or lack, of missing values can haunt an analysis. To get started we could look at known-unknowns, which are the NAs for each variable. It can be useful to use `vis_dat()` from `visdat` [@citevisdat] to get a feel for how the missing values are distributed across variables.
...
vis_dat(
  x = all_2021_ttc_data_filtered_lines,
  palette = "cb_safe"
)
```
...
We could try to quickly bring a little order to the chaos by just taking just the first word (or, the first two if it starts with "ST"). Again, we are just trying to get a sense of the data, not necessarily make binding decisions here. We use `word()` from `stringr` to extract specific words from the station names.
...
This initial exploration hints at an issue, which is there are a small number of incredibly large delays, that we might like to explore further. We will join this dataset with "delay_codes" to understand what is going on. 
...
It is extremely important that we refer to this local copy of our data when we run our scripts to explore the data, rather than asking the Inside Airbnb servers for the data each time. Indeed it might be worth even commenting out this call to their servers to ensure that we do not stress their service.
...
16. Repeat the missing data exercise conducted for the US states and population, but for the "bill_length_mm" variable in the `penguins()` dataset available from `palmerpenguins` [@palmerpenguins].
