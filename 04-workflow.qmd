...
Now that Git is set-up, we need to set-up GitHub. We created an account in [Chapter -@sec-fire-hose], which we use again here. After being signed in we first need to make a new folder, which is called a 'repo' in Git. Look for a '+' in the top right, and then select 'New Repository' (@fig-githubtwo).
...
Git and GitHub were designed for software developers, rather than data scientists. GitHub limits the size of the files it will consider to 100MB, and even 50MB will prompt a warning. Data science projects regularly involve datasets that are larger than this. In [Chapter -@sec-store-and-share] we discuss the use of data deposits, which can be especially useful when a project is completed, but when we are actively working on a project it can be useful to ignore the file, at least as far as Git and GitHub are concerned. We do this using a '.gitignore' file, in which we list all of the files that we do not want to track using Git. The [starter folder](https://github.com/RohanAlexander/starter_folder) contains an example '.gitignore' file. And it can be helpful to run `usethis::git_vaccinate()`, which will add a variety of files to a global '.gitignore' file in case you forget to do it on a project-basis.
