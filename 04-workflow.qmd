...
Code must be commented. Comments should focus on why certain code was written, (and to a lesser extent, why a common option is not selected).
...
Examples of nicely commented code include those from: [Bob Nystrom](https://github.com/wren-lang/wren/blob/46c1ba92492e9257aba6418403161072d640cb29/src/wren_value.h), @AhadyDolatsara2021, [Mathew Hauer and James Byars](https://github.com/mathewhauer/IRS-migration-data/tree/master/R), and [Burton, Jason W and Cruz, Nicole and Hahn, Ulrike](https://osf.io/zgdfr/) [@burton2021reconsidering].


Another important aspect of style is the use of a linter. This helps address stylistic issues or errors. The best way to proceed is to use `styler` [@styler]. What this does is change the code to follow a provided style guide.
...
In general we are, and will continue to be, largely concerned with just getting something done. Not necessarily getting it done in the best or most efficient way. And to a large extent, being worried about getting something done in the best or most efficient way is almost always a waste of time. Until it is not. Eventually ugly or slow code, and dogmatic insistence on a particular way of doing things, have an effect. And it is at that point that we need to be open to new approaches to ensure efficiency. These are sometimes derided as matters of taste. They are not. They are critical aspects of writing code as a professional.
...
By and large, worrying about performance is a waste of time, until it is not. For the most part we are better off just pushing things into the cloud, letting them run for a reasonable time, and using that time to worry about other aspects of the pipeline, except that becomes unfeasible. At a certain point, and this widely differs, efficiency becomes very important. There is rarely a most common area for obvious performance gains. Instead it is important to develop the ability to measure, evaluate, and think.
...
Linting\index{lint} is the process of checking code, mostly for stylistic issues, and re-arranging code to make it easier to read. (There is another aspect of linting, which is programming errors, but here we focus on stylistic issues.) In general, the best efficiency gain that we can make is to make it easier for others to read our code, even if this is just ourselves returning to the code after a break. Jane Street, the US proprietary trading firm mentioned in @@sec-clean-and-prepare, focus on ensuring their code is readable, as a core part of risk mitigation [@minsky2011ocaml]. While we may not all have billions of dollars under management, we all would likely prefer that our code does not produce errors.
...
In addition to `lintr`, we also use a styler, `styler`. This will adjust more minor aspects, such as formatting. To 
