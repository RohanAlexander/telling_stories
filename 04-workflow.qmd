...
- Reproducibility is enhanced by using Quarto, R Projects, and Git and GitHub.
    ...
    - Git and GitHub make it easier to share code and data.
        ...
        - Add your updates: 'Stage' and 'Commit'.
...
The workflow that we follow is illustrated in @fig-workflow. But it can be more concisely summarized as: 'Think an awful lot, mostly read and write, sometimes code'.

![Workflow for telling stories with data](figures/IMG_1847.png){#fig-workflow width=75% fig-align="center"}
...
An abstract is a short summary of the paper. In the default preamble, we can add a section for an abstract. Similarly, we can change the output format to ```pdf``` to produce a PDF. This uses LaTeX in the background and may require the installation of supporting packages. In particular it is common to first install the `tinytex` package [@tinytex].
```
---
title: "My document"
...
To cite R in the Quarto Document we include `@citeR`, which would put the brackets around the year, like this: @citeR or `[@citeR]`, which would put the brackets around the whole thing, like this: [@citetidyverse]. 
...
Finally, we can also cross-reference equations. To that we need to add a tag `{#eq-macroidentity}` which we then reference. For instance, we then use `@eq-macroidentity` to produce @eq-macroidentity.
...
GitHub, GitLab, and various other companies offer easier-to-use services that build on Git. While there are tradeoffs to this, we will introduce GitHub here because it is the predominant platform [@eghbal2020working, p. 21]. Git and GitHub is built into R Studio Cloud, which provides a nice option if there are issues with local installation. One of the initial challenging aspects of Git is the terminology. Folders are called 'repos'. Saving is called a 'commit'. One gets used to it eventually, but initially feeling confused is entirely normal. @happygit is especially useful for setting up and using Git and GitHub.
...
![How to access the Terminal within R Studio Cloud](figures/04-git-terminal-access.png){#fig-gitone width=95% fig-align="center"}

If you have a Mac then Git should come pre-installed and if you have Windows then it may be pre-installed. Git is pre-installed in R Studio Cloud, which provides a backup option in the case that your local machine is problematic. If you do not get a version number, then you need to install it. To do that you should follow the instructions specific to your operating system in @happygit [Chapter 5].
...
When this set-up has been done properly, the values that you entered for 'user.name' and 'user.email' will be returned to you after the last line (@fig-gitone-setup).

![Setting up Git in R Studio Cloud](figures/04-git-setup.png){#fig-gitone-setup width=95% fig-align="center"}

The details that you enter here will be public. There are various ways to hide your email address if you need to do this and GitHub provides instructions about this. @happygit [Chapter 7] provides more detailed instructions about this step, and a trouble-shooting guide


### GitHub

Now that Git is set-up, we need to set-up GitHub. We created an account in [Chapter -@sec-fire-hose], which we use again here. We first need to make a new folder, which is called a 'repo' in Git. Look for a '+' in the top right, and then select 'New Repository' (@fig-githubtwo).
...
Now returning to R Studio, in R Studio Cloud, we create a 'New Project' using 'New Project from Git Repository'. It will ask for the URL that we just copied (@fig-githubnewproject). If you are using a local machine, then this same step is accomplished through the menu: 'File' -> 'New Project...' -> 'Version Control' -> 'Git', then paste in the URL, give the folder a meaningful name, and check 'Open in new session', then 'Create Project'.

![Adding the project to R Studio](figures/04-github-new-project.png){#fig-githubnewproject width=95% fig-align="center"}

At this point, a new folder has been created locally that we can use. We will want to be able to push it back to GitHub, and for that we will need to authenticate with a Personal Access Token (PAT). To create a PAT, while signed into GitHub in the browser, run `usethis::create_github_token()` in your R session. GitHub will open in the browser with various options filled out (@fig-githubpat). It can be useful to give the PAT an informative name by replacing "Note", for instance "PAT for R Studio", then "Generate token".

![Creating a PAT](figures/04-githubpat.png){#fig-githubpat width=95% fig-align="center"}

We only have one shot to copy this token, and if we make a mistake then we will need to generate a new one. Do not include the PAT in any R script or Quarto document. Instead run `gitcreds::gitcreds_set()`, which will then prompt you to add your PAT.

To use GitHub for a project that we are actively working on we follow a procedure: 

1. The first thing to do is almost always to pull the latest changes with 'pull'. To do this, open the Git pane in R Studio, and click the blue down arrow. This pulls any changes to the folder, as it is on GitHub, into our own version of the folder.
...
3. Once this is done, we need to 'add', 'commit', and 'push'. In the Git pane, select the files to be added. This adds them to the staging area. Then click 'Commit' (@fig-githubadd). A new window will open. Add a commit message which is informative about the change that was made, and then click 'Commit' in that new window (@fig-githubcommit). Finally, click 'Push' to send the changes to GitHub. 

![Adding files](figures/04-github-add.png){#fig-githubadd width=95% fig-align="center"}

![Making a commit](figures/04-github-commit.png){#fig-githubcommit width=95% fig-align="center"}

There are a few common pain-points when it comes to Git and GitHub. We recommend committing and pushing regularly, especially when you are new to version control because this increases the number of flag posts that you could come back if needed. All commits should have an informative commit message. If you are new to version control, then the expectation of a good commit message is that it contains a short summary of the change, followed by a blank line, and then an explanation of the change including what the change is, and why it is being made. For instance, if you add graphs to a paper, then a commit message could be:
...
There is some evidence of a relationship between quality of the overall product, and commit behavior [@sprint2019mining]. In an ideal scenario the commit messages act as a kind of journal of the project.

Git and GitHub were designed for programming, rather than data science. This means that files are limited to 100MB on GitHub in terms of those that it will consider without hassle (and even 50MB will prompt a warning). Data science projects regularly have datasets that are considerably larger than this. In [Chapter -@sec-store-and-share] we discuss the use of data deposits, which can be especially useful when a project is completed, but when we are actively working on a project it can be useful to ignore the file, at least as far as Git and GitHub are concerned. We do this using a '.gitignore' file, which lists all of the files that we do not want to track using Git. The [starter folder](https://github.com/RohanAlexander/starter_folder) contains and example '.gitignore' file which may be useful to get started.
...
We used the Git pane in R Studio which reduced the need to use the Terminal, but it did not remove the need to go to GitHub and set-up a new project. Having set-up Git and GitHub, we can further improve our workflow by using `usethis` to do much of the work [@citeusethis].
