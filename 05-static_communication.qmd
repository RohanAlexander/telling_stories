...
- Only through visualization can we get a true sense of our data. Thus, we need to be comfortable with a variety of graph types, including: bar charts, scatterplots, line plots, and histograms. We can even consider a map to be a type of graph, especially after geocoding our data.
- We also must know when to summarize data, for instance using tables. Typical use cases for this include showing part of a dataset, summary statistics, and regression results. 
...
When telling stories with data, we would like to convince our our reader with data. The paper is the medium, and the data are the message. To that end, we want to show our reader the data that allowed us to come to our understanding of the story. We use graphs, tables, and maps to help achieve this. 
...
To see why graping the actual data is important, consider a dataset from `datasauRus` [@citedatasauRus]. After installing and loading the necessary packages, we can take a quick look at the dataset.
...
This is a variant of the famous "Anscombe's Quartet" which comes with R. The key takeaway is that it is important to plot the actual data and not rely  solely on summary statistics.
...
<!-- What's the point of the following paragraph? -->
<!-- It's factually correct but it doesn't feel like it integrates well into the preceding parts of the intro. -->
::: {.content-visible when-format="pdf"}
...
<!-- I would prefer using `geom_col()` in that case.  -->
<!-- This will also help you to avoid talking about the relationship between "stat" and "geom".  -->
<!-- If someone is interested in those details R4DS explains them. -->
<!-- One rarely changes the default "stat" of a geom anyway. -->
<!-- Also, an example of `geom_col()` would be good then. -->
<!-- You only hint at a potential situation where that's useful with `beps |> count(age)` but it's just as easy to just show that example. -->
...
This means that we now have three ways to combine multiple graphs: sub-figures, facets, and `patchwork`. They are useful in different circumstances. For instance, we often use 
...
<!-- When points are set to shape = 21 for example, they have both `fill` and `color` aesthetics. -->
<!-- Saying, that points can't have fill is a tiny bit misleading and might be something that could be addressed in a footnote. -->
...
<!-- Were seeds discussed in the previous chapters? -->
...
<!-- I can't really tell how beeswarms help here. I know them only for situations when the x- or y-aesthetic is used for categorical  variables.  -->
<!-- See examples in  https://github.com/eclarke/ggbeeswarm -->
...
A common use case for a scatterplot is to illustrate a relationship between two continuous variables. It can be useful to add a "summary" line using `geom_smooth()` (@fig-scattorplottwo). By default, this will use locally estimated scatterplot smoothing (LOESS) for datasets with less than 1,000 observations, but we can specify the relationship using "method", change the color with "color" and add or remove standard errors with "se". 
<!-- Will the reader understand these technical terms at this point in the book? It feels odd to  mention LOESS for less than 1000 obs here. I imagine that a beginner would not know what to do with this information. The same can be set for standard errors. -->
...
<!-- Is it really necessary to mention this over and over again? I think at this point it's okay to just use theme_minimal() and skip one default output. -->
...
<!-- Will readers understand this at this point in the book? pivot_longer() is a hard function to understand for beginners. I suggest linking to https://github.com/gadenbuie/tidyexplain/blob/main/images/tidyr-pivoting.gif -->
...
A histogram is useful to show the shape of the distribution of a continuous variable. They construct counts of the number of observations in different subsets of the support, called "bins". 
<!-- Subsets of the support. As a mathematician, I am okay with that description but I'm not sure whether everyone will understand that.
Maybe write something more descriptive: The full range of the data values is split into equal intervals/bins and the histogram counts how many observations fall into which bin.
-->
In @fig-hisogramone we examine the distribution of GDP in Ethiopia.
...
<!-- I'm not sure that the average reader identifies huge rectangle as "smooth". -->
...
<!-- But you also need to set `position = "identity"`. Otherwise, the blocks will be transparent but still stacked on each other (just like in Panel c) -->
...
<!-- This section does not actually explain what boxplots show, i.e. the quantiles. It is hinted that a boxplot shows summaries but the reader has to know what the box and the whiskers mean. -->
...
Tables are critical for telling a compelling story. Tables can communicate less information than a graph, but they do so at a high fidelity. For instance, @andersen2021presenting say that tables are especially useful to highlight a few specific values. In this book, we primarily use tables in three ways:
...
<!-- Why do we look at a  table with all NAs in one column? -->
...
In many ways maps can be thought of as another type of graph, where the x-axis is latitude, the y-axis is longitude, and there is some outline or a background image. It is possible that they are the oldest and best understood type of chart [@karsetn, p. 1].
Some ggplot pseudocode to create a map could look something like this.
...
And while there are some small complications, it is mostly as straight-forward as that.
<!-- Not sure that I agree with that.  -->
<!-- It shouldn't be taken too lightly that different map projections lead to different maps. -->
<!-- Maybe mention that in a footnote and link to some resource? -->
...
In Australia, people have to go to so-called booths in order to vote. Because the booths have coordinates (latitude and longitude), we can plot them in a map. One reason we may like to do that is to notice voting patterns w.r.t. geographies.

To get started we need to get a tile. We are going to use `ggmap` to get a tile from Stamen Maps, which builds on [OpenStreetMap](openstreetmap.org). The main argument to this function is to specify a bounding box which provides the coordinates of the edges that you are interested in. 
<!-- Maybe describe what a bounding box is? -->
