...
We can use a line plot\index{graphs!line plot} when we have variables that should be joined together, for instance, an economic time series. We will continue with the dataset from the World Bank\index{World Bank} and focus on United States GDP\index{Gross Domestic Product!United States}\index{United States!Gross Domestic Product} growth using `geom_line()` (@fig-lineplot). 
...
In many ways maps can be thought of as another type of graph, where the x-axis is latitude, the y-axis is longitude, and there is some outline or a background image.\index{tables!maps}\index{maps} It is possible that they are the oldest and best understood type of chart [@karsetn, p. 1]. We can generate a map in a straight-forward manner. That said it is not to be taken lightly; things quickly get complicated!
...
Once you have defined the bounding box, then the function `get_stamenmap()` will get the tiles in that area (@fig-heyitscanberra).\index{maps!tiles} The number of tiles that it needs depends on the zoom, and the type of tiles that it gets depends on the type of map. We have used a black and white map but there are others including color and terrain. We pass the tiles to `ggmap()` which will plot the tiles. You need an internet connection for this to work as `get_stamenmap()` will be actively downloading these tiles.\index{internet!map tiles}
...
To see another example of a static map we will plot some data on United States military deployments from `troopdata`.\index{maps!US military bases} We can access data about United States overseas military bases back to the start of the Cold War using `get_basedata()`.
...
1. *(Plan)* Consider the following scenario: *Three friends---Edward, Hugo, Lucy---each measure the height of 20 of their friends. Each of the three use a slightly different approach to measurement and so make slightly different errors.* Please sketch out what that dataset could look like and then sketch a graph that you could build to show all observations.
...
2. Assume the `tidyverse` and the "beps" dataset have been installed and loaded. Which argument should be added to `geom_bar()` in the following code to make the bars for the different parties be next to each other rather than on top of each other `beps |> ggplot(mapping = aes(x = age, fill = vote)) + geom_bar()`?
