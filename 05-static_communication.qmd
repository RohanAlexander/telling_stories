...
We can use this to get a table such as @tbl-testdatasummarynormal. That might be useful for exploratory data analysis, which we cover in @sec-exploratory-data-analysis. (Here we remove population to save space.)
...
9. Would this code `data |> ggplot(aes(x = col_one)) |> geom_point()` if we assume the dataset and columns exist (pick one)?
    a. Yes
    b.  No
