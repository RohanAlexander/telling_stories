...
13. From @healyviz, how do you add some transparency to the points when using `geom_point()` (pick one)?
    a.  By setting `alpha` in  to a value between 0 and 1.
    b. By removing geom_point() from the plot.
    c. By using `color = NULL` in `aes()`
14. From @r4ds, what does `labs()` do in `ggplot2` (pick one)?
...
23. Would this code `data |> ggplot(aes(x = col_one)) |> geom_point()` work if we assume the dataset and columns exist (pick one)?
    a.  No.
    b. Yes.
...
30. What does adjusting the number of bins, or changing the binwidth, affect for a histogram (pick one)?
    ...
    d.  The smoothness of the distribution.
