...
We are often interested in the relationship between two numeric or continuous variables. We can use scatterplots\index{graphs!scatterplot} to show this. A scatterplot may not always be the best choice, but it is rarely a bad one [@weissgerber2015beyond]. Some consider it the most versatile and useful graph option [@historyofdataviz, p. 121]. To illustrate scatterplots, we use `WDI` [@WDI] to download some economic indicators from the World Bank\index{World Bank}, and in particular `WDIsearch()` to find the unique key that we need to pass to `WDI()` to facilitate the download.
...
We illustrate showing part of a dataset using `kable()` from `knitr`. We use the World Bank\index{World Bank} dataset that we downloaded earlier and focus on inflation, GDP growth, and population as unemployment data are not available for every year for every country.\index{tables!knitr::kable()}
...
To see another example of a static map we will plot some data on United States military deployments from `troopdata` [@troopdata].\index{maps!United States military deployments} We can access data about United States overseas military bases back to the start of the Cold War using `get_basedata()`.
