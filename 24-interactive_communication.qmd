...
By default a basic website is included and this can be produced with "Build" -> "Render Website" (@fig-quartotwo). By default it will show in the "Viewer" pane, but can also be shown in a New Window. Again, at this point we may like to change the details to reflect our own. In particular, we may like to change the title of "index.qmd", and add our own details (@fig-quartothree).
...
After the details are personalized and we are not unhappy with the website it can be pushed to GitHub and then hosted with GitHub Pages. When we use this service, by default, GitHub would try to build the site, which we do not want, so we need to first add a hidden file to turn that off, by running this in the console:
