...
sim_run_data <-
  ...
  mutate(
    ...
    weather_conditions = 
      case_when(
        was_raining == "No" & humidity == "Low" ~ "No rain, low humidy",
        was_raining == "Yes" & humidity == "Low" ~ "Raining, low humidy",
        was_raining == "No" & humidity == "High" ~ "No rain, high humidy",
        was_raining == "Yes" & humidity == "High" ~ "Raining, high humidy"
      )
  )
...
We only want respondents who are registered to vote, and we are only interested in those that voted for either Biden or Trump. So we can filter to only those respondents and then add more informative labels.
...
In the end we are left with 43,554 respondents (@fig-cesissogooditslikecheating). Gender of "female" and "male" is what is available from the CES.
...
We are interested in whether we can forecast who a respondent is likely to vote for, based only on knowing their highest level of education and gender. One approach is to use `tidymodels` to build a prediction-focused logistic regression model. 
...
Finally, we use `fit_resamples()` to fit our model across the different combinations of folds.
...
The average accuracy of our across the folds is 0.61, while the average sensitivity is 0.19 and the average specificity is 0.90.
...
```{r}
add_predictions <- collect_predictions(ces2020_cross_validation)
add_predictions
```
