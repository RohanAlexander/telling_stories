...
For ease of next steps in this chapter the notation for probability distributions follows @pitman, the notation for frequentist statistics and machine learning follows @islr, and the notation for Bayesian statistics follows @citemcelreath.
...
The intercept is the marathon time associated with a hypothetical five-kilometer time of 0 minutes. Hopefully this example illustrates the need to always carefully interpret the intercept coefficient. And to disregard it at times.
...
We want to try to speak to the "true" relationship, so we need to try to capture how much we think our understanding depends on the sample that we have to analyze. And this is where the standard error comes in. It tells us how wrong our estimates are compared with the actual data (@fig-fivekmvsmarathon-3).
...
@tbl-modelsummarylogistic makes it clear that each of the approaches is similar in this case. They agree on the direction of the effect of seeing an extra car on the probability of it being day. And even the magnitude of the effect is estimated to be similar.
...
To illustrate the situation, we could simulate data about the number of A's that are awarded in each university course. In this simulated example, we consider three departments, each of which has a number of courses. Each course will award a different number of A's.
...
count_of_A |>
  ...
  labs(
    x = "Number of A's awarded",
    y = "Numnber of classes",
    fill = "Department"
  ) +
  theme_classic() +
  scale_fill_brewer(palette = "Set1")
...
Our simulated dataset has the number of A's awarded by courses, which are structured within departments (@fig-simgradesdepartments). (In @sec-multilevel-regression-with-post-stratification, we will take advantage of this departmental structure, but for now we just ignore it.) 
...
\end{aligned}
$$
where $y_i$ is the number of A's awarded, and $x_1$ is the course.

We can use `glm()` from base R to get a quick sense of the data. The estimates are contained in the first column of @tbl-modelsummarypoisson.
...
As with logistic regression, the interpretation of the coefficients from Poisson regression can bes difficult. In particular, the interpretation of the coefficient on "department2" is that it is the log of the expected difference between departments. We expect $e^{0.7893} \approx 2.2$ and $e^{1.4804} \approx 4.4$ additional A's in departments 2 and 3, compared with department 1.
...
As with logistic regression, we can use `marginaleffects()` from [@marginaleffects] to help with interpreting these results. In particular, it may be useful to consider how we expect the number of A's to change as we go from one department to another. @tbl-marginaleffectspoisson suggests that in our dataset classes in Department 2 tend to have around five additional A's, compared with Department 1. And that classes in Department 3, tend to have around 17 more A's, compared with Department 1. 

```{r}
#| label: tbl-marginaleffectspoisson
#| tbl-cap: "The estimated difference in the number of A's awarded at each department"

marginaleffects::marginaleffects(grades_rstanarm) |> 
  summary() |> 
  select(-type, -term) |> 
  knitr::kable(
    digits = 2,
...
In an earlier age, @edgeworth1885methods made counts of the dactyls in Virgil's Aeneid (@HistData makes this dataset easily available with `HistData::Dactyl`). Inspired by this we could use `gutenbergr` [@gutenbergr] to get the text of *Jane Eyre* by Charlotte Brontë. We could then consider the first 10 lines of each chapter, and count the number of times either "E" or "e" appears.
...
We are interested in only those lines that have content. And so we remove those empty lines that are just there for spacing. Then we can create counts of the number of times "e" or "E" occurs in that line, for the first 10 lines of each chapter. @tbl-countejane finds that there are 5 e/E's in the first line and 8 in the second.
...
```{r}
#| label: tbl-countejane
#| tbl-cap: "Counts of e/E's by line in Jane Eyre"

jane_eyre_reduced |> 
  select(chapter, count_e, text) |> 
  head() |> 
  knitr::kable(
    col.names = c("Chapter", "Number of e/E", "Text"),
    booktabs = TRUE,
    linesep = ""
    )
```

We can verify that the mean and variance of the number of E/e's is roughly similar by plotting all of the data (@fig-janeecounts). In particular, the mean, in pink, is 6.9, and the variance, in blue, is 6.2. While they are not entirely the same, they are similar.
...
\end{aligned}
$$
where $x_1$ is the chapter.

The estimates, using `rstanarm()`, are in @tbl-modelsummarypoisson.
...
@fig-predictionsjaneecounts shows the the number of e/E's predicted by the model, for each chapter.

```{r}
#| label: fig-predictionsjaneecounts
#| fig-cap: "The predicted number of e/E's in each chapter"

marginaleffects::plot_cap(jane_e_counts, condition = "chapter") +
  labs(x = "Chapter",
