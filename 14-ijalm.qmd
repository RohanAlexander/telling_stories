...
We can add additional explanatory variables to `lm()` with `+` (@tbl-modelsummaryraining).

```{r}
#| include: true
#| label: tbl-modelsummaryraining
#| tbl-cap: "Explaining marathon times based on five-kilometer run times and whether it is raining"
...
When we look at the training and test datasets, we can see that we have placed most of our dataset into the training dataset. We will use that to estimate the parameters of our model. We have kept a small amount of it back, and we will use that to evaluate our model (@tbl-modelsummarytidymodels).

```{r}
#| label: tbl-modelsummarytidymodels
#| tbl-cap: "Forecasting marathon times based on five-kilometer run times"
...
modelsummary(simulated_running_data_first_model_tidymodels)
```

We will use `tidymodels` for forecasting. But when we are focused on inference, instead, we will use Bayesian approaches. To do this we use the probabilistic programming language "Stan", and interface with it using `rstanarm` [@citerstanarm]. This is slightly simplistic---we could do forecasting with Bayesian models, and we could  adapting Bayesian approaches within `tidymodels`. But, for the sake of simplicity and clarity, we keep these separated in this way for this book .
...
Finally, the issue of picking priors is a challenging one and the subject of extensive study. For the purposes of this book, just using the default is fine. But even if they are just the default, they must be explicitly specified in the model and included in the function. We can use `prior_summary()`, to find out which priors were used.
...
modelsummary(day_or_night_model)
```
...
modelsummary(day_or_night_tidymodels)
```
...
modelsummary(day_or_night_rstanarm,
             statistic = 'conf.int')
```
...
In a sense, we started with a cloud option, through our initial recommendation, in @sec-fire-hose of using RStudio Cloud, before we moved to our local computer in @sec-r-essentials. That cloud option was specifically designed for beginners. We will now introduce 
...
analogsea::debian_apt_get_install(
  id,
  "libssl-dev",
  "libsodium-dev",
