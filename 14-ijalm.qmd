...
- Finally, putting models into production requires a different set of skills to building a model. We need a familiarity with some cloud provider, APIs, and of course modelling. The biggest difficulty can be getting things set-up.
...
mean_deaths <- mean(alberta_cod$total_deaths)
var_deaths <- var(alberta_cod$total_deaths)

alberta_cod |> 
  ggplot(aes(x = total_deaths, y= cause)) +
  geom_point(alpha = 0.5) +
  geom_vline(xintercept = mean_deaths, linetype = "dashed", color = "#C64191") +
  geom_vline(xintercept = var_deaths, linetype = "dashed", color = "#0ABAB5") +
  theme_minimal() +
  labs(
    x = "Number of deaths",
    y = "Cause"
...
One thing that we notice is that the mean, in pink, is quite different to the variance.
