...
The intercept is the marathon time associated with a hypothetical five-kilometer time of 0 minutes. Hopefully this example illustrates the need to always interpret the intercept coefficient carefully. And to disregard it at times.
...
We can verify that the mean and variance of the number of E/e's is roughly similar by plotting all of the data (@fig-janeecounts). The mean, in pink, is 6.9, and the variance, in blue, is 6.2. While they are not entirely the same, they are similar.
...
\end{aligned}
$$
where $x_1$ is the chapter. We could estimate the parameters using `stan_glm()`. 
...
We download that data, and save it as "alberta_COD.csv". We can look at the distribution of these deaths, by year and cause, where we have truncated the cause of death because some are quite extensive (@fig-albertacod). (Note that because some causes are not always in the top 30 each year, not all causes have the same number of occurrences.)
...
One thing that we notice is that the mean, 665, is different to the variance, 227,152 (@tbl-ohboyalberta).
