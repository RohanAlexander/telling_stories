...
# It's Just A Linear Model {#sec-its-just-a-linear-model}
...
This results in one 'linear best-fit' line (@fig-fivekmvsmarathonwithbestfit), but it is worth thinking about all the assumptions and decisions that it took to get us to this point.
...
To this point, we have just had a quick look at the regression results using `summary()`. A better approach is to use `modelsummary()` from `modelsummary` [@citemodelsummary] (@tbl-modelsummaryruntimes).

```{r}
#| label: tbl-modelsummaryruntimes
#| tbl-cap: "Explaining marathon time based on five-kilometer run times and weather features"
...
#| include: true
#| fig-label: poissondistributiontakingshape
#| message: false
#| warning: false
...
In a sense, we started with a cloud option, through our initial recommendation, in Chapter \@ref(drinking-from-a-fire-hose) of using R Studio Cloud, before we moved to our local computer in Chapter \@ref(r-essentials). That cloud option was specifically designed for beginners. We will now introduce 
