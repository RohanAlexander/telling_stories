...
ces <- 
  ...
  filter(votereg == 1,
         CC20_410 %in% c(1, 2)) |> 
  select(CC20_410, gender, educ)

write_csv(ces, "inputs/data/ces2020.csv")
```


```{r}
ces <- read_csv("inputs/data/ces2020.csv")
...
ces_split <- initial_split(ces, prop = 0.80)
ces_train <- training(ces_split)
ces_test <- testing(ces_split)

ces_tidymodels <-
  logistic_reg(mode = "classification") |>
  set_engine("glm") |>
  fit(voted_for ~ gender + educ,
      data = ces_train)
```
