...
The intercept is the marathon time that we would expect with a five-kilometer time of 0 minutes. Hopefully this example illustrates the need to carefully interpret the intercept coefficient! 

Following @gelmanhillvehtari2020 [p. 84] we recommend considering the coefficients as comparisons, rather than effects, and to use language that makes it clear these are comparisons, on average, that are based on one dataset. For instance, we may say that the coefficient on the five-kilometer run time shows how different individuals compare. When comparing the marathon times of individuals in our dataset whose five-kilometer run time differed by one minute, on average, we would expect their marathon times to differ by about eight minutes. This makes sense seeing as a marathon is roughly that many times longer than a five-kilometer run.
...
The result (@tbl-modelsummaryruntimes) shows that we find that comparing individuals who ran in the rain with those who did not, the ones in the rain tended to have a slower time. And this corresponds with what we expect if we look at a plot of the data (@fig-fivekmvsmarathonbinary-1).

In addition to wanting to include additional explanatory variables, we may think that they are related to each another. For instance, maybe rain really matters if it is also humid that day. So we are interested in the humidity and temperature, but also how those two variables interact (@fig-fivekmvsmarathonbinary-2). We can do this by using `*` instead of `+` when we specify the model. When we interact variables in this way, then we almost always need to include the individual variables as well and `lm()` will do this by default (@tbl-modelsummaryruntimes).
...
@breiman2001statistical describes two cultures of statistical modelling: one focused on prediction and the other on inference. At the time, and speaking in generalizations, various disciplines tended to focus on either inference or prediction. The rise of data science has meant there is now a need to be comfortable with both [@Neufeld2021]. The two cultures are being brought closer together, their separate evolution means there are still considerable differences.

We will use `tidymodels` when we are interested in prediction. And when we are focused on inference, instead, we will use the probabilistic programming language Stan, and interface with it using `rstanarm` [@citerstanarm]. This is slightly simplistic---we could do prediction with Bayesian models, and we could use Stan approaches within `tidymodels`. But, for the sake of simplicity and clarity, we keep these separated in this way for this book. It is important to be conversant in both ecosystems and cultures.
...
When we look at the training and test datasets, we can see that we have placed most of our dataset into the training dataset. We will use that to estimate the parameters of our model. These are summarized in the first column of @tbl-modelsummarybayesbetter. We have kept a small amount of it back, and we will use that to evaluate our model.
...
The estimation results, which are in the second column of @tbl-modelsummarybayesbetter, indicate that the estimation is not quite what we expect. For instance, the comparison between individuals is estimated to be around three, which seems low given the ratio of five kilometer to marathon distance.
...
We will not make much of it in this book, but we will be interested in understanding what the priors imply before we involve any data. We do this by implementing prior predictive checks. In particular, simulating from the priors to look at what they imply about our coefficients. This process is no different to all the other simulation that we have done to this point.
...
priors <-
  tibble(
    ...
    beta_0 = rep(rnorm(n = number_of_draws, mean = 0, sd = 2.5), times = 16),
    beta_1 = rep(rnorm(n = number_of_draws, mean = 0, sd = 2.5), times = 16),
...
We will simulate data on whether it is day or night, based on the number of cars on the road.
...
As with linear regression, when it comes to logistic regression we can use `glm()` from base to put together a quick model. In this case we will try to work out whether it is day or night, based on the number of cars we can see. We are interested in estimating @eq-logisticexample:
$$
...
We could use `tidymodels` to run this if we wanted. In order to do that, we first need to change the class of our dependent variable into a factor.
...
As before, we can make a graph of the actual results compared with our estimates. But one nice aspect of this is that we could use our test dataset to more thoroughly  evaluate our model's forecasting ability, for instance through a confusion matrix. We find that the model does well on the held-out dataset. In particular, there were 90 observations where the model predicted it was day, and it was actually day, and 95 where the model predicted it was night, and it was night. It was wrong for a total of 15 observations and these were split across 7 where it thought it was night, but it was day, and 8 where it was the opposite case.
...
We might be interested in inference, and so want to build a Bayesian model using `rstanarm`. Again, we will more specify our model, but just use the default priors that `rstanarm` uses:
...
We only want respondents who are registered to vote, and we are only interested in those that voted for either Biden or Trump. So we can filter to only those respondents and then add more informative labels. Gender of "female" and "male" is what is available from the CES.
...
ces_tidymodels <-
  logistic_reg(mode = "classification") |>
  set_engine("glm") |>
  fit(voted_for ~ gender + education,
      data = ces2020_train)
...
When we introduced `tidymodels`, we discussed the importance of randomly constructing training and test sets. We use the training dataset to estimate parameters, and then evaluate the model on the test set. It is natural to ask why should we be subject to the whims of randomness and whether we are making the most of our data. For instance, what if a good model is poorly evaluated because of some random inclusion in the test set? And what if we do not have a large test set?
...
<!-- And we could look at which observations are in each fold with `analysis()`. -->

<!-- ```{r} -->
<!-- # ces2020_10_folds$splits[[1]] |> analysis() -->
<!-- # ces2020_10_folds$splits[[2]] |> analysis() -->
<!-- ``` -->

We then fit our model across the different combinations of folds with `fit_resamples()`. So in this case, the model will be fit 10 times.
...
What does this mean? Accuracy is the generalized proportion of individuals that were correctly classified. So the result of 0.61 suggests the model is doing better than a coin toss, but not much more. Sensitivity is the percentage of true that are identified as true [@islr, p. 145]. So in this case would mean that the model predicted a respondent voted for Biden and they actually did. And specificity is the percentage of false that are identified as false [@islr, p. 145]. So in this case it is the percentage of voters that voted for Trump, that were predicted to vote for Trump.  It is clear that the model is having trouble identifying Biden supporters. 

We can see this in more detail by looking at the confusion matrix [@tbl-metricsvoters-2]. When used with resampling approaches, such as cross-validation, the confusion matrix if computed for each fold and then averaged.
...
#| tbl-cap: "The performance of the model is poor"
