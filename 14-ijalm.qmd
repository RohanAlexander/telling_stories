...
- Linear models are a key component of statistical inference and enable us to quickly model a wide range of data. Simple and multiple linear regression model a continuous dependent variable as a function of one, and multiple, independent variables, respectively. Linear regression can be generalized for alternative types of dependent variables. For instance, logistic and Poisson regression have a binary and integer count dependent variable, respectively.
- Linear and generalized linear models and can be used for making predictions on new data without a known label (machine learning), or doing inference on the coefficients of the model. In this book, we will be concerned with one of two different aspects: prediction or inference. We will use machine learning approaches for the former, and Bayesian methods for the latter. 
...
Comment^[I FEEL LIKE THE AWS SECTION SHOULD BE IN THE APPENDIX. ITS PROVIDING A LOT OF NITTY-GRITTY DETAILS WHICH MAY BECOME OUTDATED OVERTIME AS WEB SERVICES CHANGE OPTION SETS, ETC.]
...
Comment^[I WOULD EXPLAIN WHAT EXACTLY APIS ARE FOR AND WHY THEY ARE USEFUL, ALSO I THINK THIS SHOULD BE A DIFFERENT SECTION. BUT I WOULD KEEP LINEAR REG & GLM IN THE SAME SECTION TO BE CLEAR. THE IDEA OF DEPLOYING A MODEL IS COMPLETELY INDEPENDENT ABOUT WHETHER IT IS LINEAR OR NOT]
