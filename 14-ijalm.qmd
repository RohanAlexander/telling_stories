...
estimated_standard_deviation <-
  sqrt(
    sum(normal_example$diff_square) / nrow(normal_example)
  )
...
We do not, and cannot, know the "true" relationship between $X$ and $Y$. All we can do is use our sample of data to estimate it. But because our understanding depends on that sample, for every possible sample, we would get a slightly different relationship, as measured by the coefficients. 
...
We can add additional explanatory variables to `lm()` with `+` (@tbl-modelsummaryruntimes). Again, we will include a variety of quick tests for class and the number of observations, and also add another about missing values.

```{r}
stopifnot(
  class(simulated_running_data$marathon_time) == "numeric",
  class(simulated_running_data$five_km_time) == "numeric",
  class(simulated_running_data$was_raining) == "character",
  all(complete.cases(simulated_running_data)),
  nrow(simulated_running_data) == 100
)
...
stopifnot(
  ...
  class(simulated_running_data$five_km_time) == "numeric",
  class(simulated_running_data$was_raining) == "character",
  class(simulated_running_data$humidity) == "character",
  all(complete.cases(simulated_running_data)),
  nrow(simulated_running_data) == 100,
  unique(simulated_running_data$humidity) %in% c("Low", "High")
)
...
simulated_running_data_first_model_rstanarm <-
  readRDS(file = "simulated_running_data_first_model_rstanarm.rds")
```
...
simulated_running_data_second_model_rstanarm <-
  stan_glm(
    formula = marathon_time ~ five_km_time + was_raining,
    data = simulated_running_data,
    family = gaussian(),
...
saveRDS(simulated_running_data_first_model_rstanarm,
        file = "simulated_running_data_second_model_rstanarm.rds")
```
...
saveRDS(
  simulated_running_data_second_model_rstanarm,
  file = "outputs/model/simulated_running_data_second_model_rstanarm.rds"
)
```
...
simulated_running_data_second_model_rstanarm <-
  readRDS(file = "outputs/model/simulated_running_data_second_model_rstanarm.rds")
```
...
modelsummary(
  list(
    "tidymodels" = simulated_running_data_first_model_tidymodels,
    "rstanarm" = simulated_running_data_first_model_rstanarm,
    "rstanarm better priors" = simulated_running_data_second_model_rstanarm
  ),
  statistic = 'conf.int'
)
```
...
```{r}
prior_summary(simulated_running_data_second_model_rstanarm)
```
...
pp_check(simulated_running_data_second_model_rstanarm) +
  theme_classic()

posterior_vs_prior(simulated_running_data_second_model_rstanarm) +
  theme_minimal() +
  scale_color_brewer(palette = "Set1")
```
