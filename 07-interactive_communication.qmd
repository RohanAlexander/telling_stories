...
![Example settings for setting up `distill`](figures/distill_one.png){#fig-distillone width=90% fig-align="center"}
...
The trestles page will open, and we need to add the following line in the yaml file: `site: distill::distill_website`. In @fig-distillthree it was added at line 16, and then we can rebuild the website.
...
`shiny` [@citeshiny] is a way of making interactive web applications using R. It is fun, but fiddly. Here we are going to step through one way to take advantage of `shiny`. Which is to quickly add some interactivity to our graphs. We will return to `shiny` in [Chapter -@sec-deploying-models].
