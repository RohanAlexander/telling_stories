...
Coherence means that the values in the dataset are not obviously wrong. For instance, currencies should likely not have alphabetical values in them; names should likely not have numbers; and velocities should not be faster than the speed of light. Internal consistency means that we ensure the dataset is consistent with itself. For instance, if American spelling is being used in a book, then we would expect that American spelling is used throughout. External consistency means that the dataset is consistent with external sources. For instance, if our dataset purports to be about populations of cities in a country, then we would expect that they are consistent, at least roughly, with, say, those available relevant censuses.

SpaceX controls their rockets using a cycle of either 10 or 50 Hertz (equivalent to 0.1 and 0.02 seconds, respectively). Each cycle inputs from sensors, such as temperatures and pressures, are read, processed, and used to write outputs, such as 'open or close a valve' [@martinpopper]. This cyclical process of small adjustments is what we use in data cleaning cleaning and preparation as well. Rather than trying to make everything perfect from the start, we get started, and make adjustments on an iterative basis in a way that reflects a Kaizen process, or one of continuous improvement.
...
Machine readable names is an easier standard to meet, but usually means avoiding spaces and special characters. A space can be replaced with a underscore. Usually, special characters should be removed because they can be inconsistent between different computers and languages. This is especially the case with slash and backslash, asterisk, and single and double quotation marks, none of which should, almost, ever be used in names. Names should also be unique within a dataset, and unique within a collection of datasets unless that particular column is being deliberately used as a key to join different datasets. This usually means that the domain is critical for effective names, and when working as part of a team this all gets much much difficult [@hermans2017peter].
