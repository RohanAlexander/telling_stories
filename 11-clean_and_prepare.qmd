...
We may not always be allowed to share that local copy, but we can almost always create it. If we are not even able to create a local copy of the raw data, then it may be that the best we can do is save a copy of as close to the raw data as is possible, create a simulated version of the raw data that conveys the main features, and include detailed access instructions in the README.
...
tidy_data <-
  ...
  separate(
    ...
    sep = " (?=[[:digit:]])"  # Separate according to...
  ) |>
  select(-drop_me)
...
11. When dealing with ages, what is a possible class for the variable (select all that apply)?
    ...
    c. numeric
    d. factor
12. Please consider the cities in Germany. Use `testthat` to define three tests that could apply.
