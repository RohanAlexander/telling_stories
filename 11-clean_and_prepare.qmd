...
In this chapter we put in place more-formal approaches for data cleaning and preparation. It is centered around:

1. coherence;
2. internal consistency; and
3. external consistency.

Coherence means that the values in the dataset are not obviously wrong. For instance, currencies should likely not have alphabetical values in them; names should likely not have numbers; and velocities should not be faster than the speed of light. Internal consistency means that we ensure the dataset is consistent with itself. For instance, if American spelling is being used in a book, then we would expect that American spelling is used throughout. In the context of a dataset, that might mean that constituent columns actually sum up to the total column. External consistency means that the dataset is consistent with outside sources. For instance, if our dataset purports to be about populations of cities in a country, then we would expect that they are consistent, at least roughly, with, say, those available relevant censuses.

SpaceX, the US rocket company, controls their rockets using cycles of 10 or 50 Hertz (equivalent to 0.1 and 0.02 seconds, respectively). Each cycle inputs from sensors, such as temperatures and pressures, are read, processed, and used to write outputs, such as 'open or close a valve' [@martinpopper]. We use a similar iterative process of small adjustments when data cleaning and preparation as well. Rather than trying to make everything perfect from the start, we get started, and adjust on an iterative basis, making continuous improvements.

To a large extent, the role of data cleaning and preparation is so great that the only people that we can trust understand their data, are those that who have cleaned it. And, paradoxically, often those that do the cleaning and preparation are often those that trust it the least. At some point in every data science workflow, those doing the modelling should get their hands dirty with data cleaning. To clean and prepare data is to make many decisions, some of which may have important effects on our results. For instance, @labelsiswrongs find the test sets of popular datasets in computer science contain, on average, around 3 per cent labelling errors and that this has the potential to result in incorrect model selection.
...
Character encoding is needed for computers, which are based on strings of 0s and 1s, to be able to consider symbols such as alphabets. One source of particularly annoying data cleaning issues is different character encodings, particularly when dealing with foreign languages or odd characters. In general we want to use UTF-8, and you find the encoding of a character vector using `Encoding()`.
