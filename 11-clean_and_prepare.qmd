...
Our purpose, during data cleaning and preparation\index{data cleaning and preparation}, is to then bring our raw data close to that plan. Ideally, we would plan so that the desired endpoint of our dataset is "tidy data"\index{tidy data}. This was introduced in @sec-r-essentials, but briefly, it means that [@r4ds; @wickham2014tidy, p. 4]:
...
expect_length(check_me, 1)  # Is the dataset of length one?
expect_equal(class(cleaned_data$names), "character")  # Are all of the observations characters?
expect_equal(unique(cleaned_data$names), "Patricia")  # Is every unique oservation "Patricia"?
```
...
We begin by downloading the data.^[If the DHS link breaks then replace their URL with: https://www.tellingstorieswithdata.com/inputs/pdfs/1996_Tanzania_DHS.pdf.] 
...
In the Tanzanian example, we were interested in only one page, so we made our modifications directly to that page. Here we want to consider many pages, so we instead write a function, as introduced in @sec-r-essentials, and then apply it to many pages. The first challenge is to get the dataset into a format that we can more easily manipulate. We will consider each page of the PDF and extract the relevant parts. To do this, we first write a function, and then apply it to each page. 
...
The PDF is made-up of three long tables. So, we can first get the names of the counties based on those final two tables and then reconcile them to get a full list of county names.
