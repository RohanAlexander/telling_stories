...
In this case, exercise is called a "collider" and if we were to condition on it, then we would create a misleading relationship.\index{Directed Acyclic Graph!collider} Income influences exercise, but a person's happiness also affects this. Exercise is a collider because both the independent and dependent variable of interest influence it.
:::
...
professionals <-
  tibble(
    VO2 = runif(n = num_pros, min = 0.7, max = 0.9),
    chance_of_winning = runif(n = num_pros, min = 0.7, max = 0.9),
    type = "Professionals"
  )

general_public <-
  tibble(
    VO2 = runif(n = num_public, min = 0.6, max = 0.8),
    noise = rnorm(n = num_public, 0, sd = 0.03),
...
newspapers <-
  ...
  mutate(across(c(id_news, after_national, local, national), 
                as.factor)) |>
...
purchase_data <- 
  ...
  mutate(# random draw whether free shipping
    free_shipping = rbinom(n = sample_size, size = 1, 
                           prob = softmax_prob),) |>
...
purchase_data |> 
  summarise(average_spend = 
              round(mean(spend), 2), .by = free_shipping) |> 
  mutate(free_shipping = 
           if_else(free_shipping == 0, "No", "Yes")) |> 
  kable(
...
6. What is the key assumption in difference-in-differences
    a. Parallel trends.
    b. Heteroscedasticity.
7. According to @Meng2021What "Data science can persuade via..." (pick all that apply):
...
8. According to @riedererdesignpatterns if I have "disjoint treated and untreated groups partitioned by a sharp cut-off" then which method should I use to measure the local treatment effect at the juncture between groups (pick one)?
    a. regression discontinuity
    b. matching
    c. difference-in-differences
    d. event study methods
9. What does causal inference require according to @riedererdesignpatterns (pick all that apply)?
    a.  data management
    b.  domain knowledge
    c.  probabilistic reasoning
10. Consider an Australian 30-39 year old male living in Toronto with two children and a PhD. Which of the following do you think they would match most closely with and why (please explain in a paragraph or two)?
...
11. What is propensity score matching? If you were matching people, then what are some of the features that you would like to match on? What sort of ethical questions does collecting and storing such information raise for you?
12. What is a DAG (in your own words please)?
13. Draw a DAG illustrating the collider bias described by @bronnerftw.
14. @NoiseKahneman say "...while correlation does not imply causation, causation does imply correlation. Where there is a causal link, we should find a correlation". With reference to @Cunningham2021 [Chapter 1], are they right or wrong, and why?
