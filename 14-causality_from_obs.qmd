...
There are 1,196 observations in the dataset and 52 variables. @angelucci2019newspapers are interested in the 1960-1974 time-period which has around 100 newspapers. There are 14 national newspapers at the beginning of the period and 12 at the end. The key period is 1967, when the French government announced it would allow advertising on television. @angelucci2019newspapers argue that national newspapers were affected by this chance, but local newspapers were not. The national newspapers are the treatment group and the local newspapers are the control group.
...
In general, we can replicate the main results of @angelucci2019newspapers and find that in many cases there appears to be a difference from 1967 onward. @angelucci2019newspapers [pp. 353-358] also includes an excellent example of the discussion of interpretation, external validity, and robustness that is required for difference in differences models.
...
Regression discontinuity design (RDD) was established by @thistlethwaite1960regression and is a popular way to get causality when there is a continuous variable with cut-offs that determine treatment. Is there a difference between a student who gets 79 per cent and a student who gets 80 per cent? Probably not much, but one may get an A-, while the other may get a B+, and seeing that on a transcript could affect who gets a job which could affect income. In this case the percentage is a "forcing variable" or "forcing function" and the cut-off for an A- is a "threshold". As the treatment is determined by the forcing variable we need to control for that variable. And these seemingly arbitrary cut-offs can be seen all the time. Hence, there has been a great deal of work using RDD.
...
6. Please read @markupinsurance and discuss two statistical aspects, and two ethical aspects..
...
19. What is the main reason that @oostrom2021 finds that the outcome of an RCT can depend on who is funding it (pick one)?
    a. Publication bias
...
26. According to @riedererdesignpatterns (pick all that apply):
    a. data management
    b. domain knowledge
