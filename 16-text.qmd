...
The packages that we need in this chapter are below. `astrologer` is not on CRAN, so we need to install it using: `devtools::install_github("sharlagelfand/astrologer")`.\index{packages!GitHub installation}
...
We now use `astrologer` [@astrologer], which is a dataset of horoscopes to explore a real dataset.\index{zodiac}\index{astrology} 
