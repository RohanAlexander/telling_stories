...
Text is all around us.\index{text as data} In many cases, text is the earliest type of data that we are exposed to. Increases in computational power, the development of new methods, and the enormous availability of text means that there has been a great deal of interest in using text as data. Using text as data opens up a wide opportunity for unique analyses. For instance:

- text analysis of state-run newspapers in African countries can identify manipulation by governments [@Hassan2022]; 
- the text from UK daily newspapers can be used to generate better forecasts of GDP and inflation [@Kalamara2022], and similarly, the New York Times\index{New York Times} can be used to create an uncertainty index which correlates with US economic activity [@Alexopoulos2015]; 
...
Text modelling is an exciting area of research.\index{text!cleaning and preparation} But, and this is true more generally, the cleaning and preparation aspect is often at least as difficult as the modelling.\index{data cleaning and preparation} We will cover some essentials and provide a foundation that can be built on.
...
We typically want to construct a document-feature matrix, which has documents in each observation, words in each column, and a count for each combination, along with associated metadata.\index{document-feature matrix} For instance, if our corpus was the text from Airbnb\index{Airbnb} reviews, then each document may be a review, and typical features could include "The", "Airbnb", "was", "great". Notice here that the sentence has been split into different words. We typically talk of "tokens" to generalize away from words, because of the variety of aspects we may be interested in, but words are commonly used.\index{tokens}
...
We use the tokens in the corpus to construct a document-feature matrix (DFM) using `dfm()` from `quanteda` [@quanteda].\index{document-feature matrix}
...
In that case, we knew what the tuples were. But it might be that we were not sure what the common tuples were in the corpus. We could use `tokens_ngrams()` to identify them. We could ask for, say, all bi-grams in an excerpt from the book *Jane Eyre*.\index{Brontë, Charlotte!Jane Eyre} We showed how to download the text of this book from Project Gutenberg\index{Project Gutenberg} in @sec-its-just-a-generalized-linear-model and so here we load the local version that we saved earlier.
...
We can see that there are 106 horoscopes for each zodiac sign. In this example we first tokenize by word, and then create counts based on zodiac sign only, not date. We use `tidytext` because it is used extensively in @hvitfeldt2021supervised.
...
7. What is the sixth-most common word used in the zodiac signs for pisces, that is unique to that sign?
    ...
    c. fishes
    d. pisces
8. Re-run the Canadian topic model model, but only including five topics. Looking at the words in each topic, how would you describe what each of them is about?
