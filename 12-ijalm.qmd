...
- Linear models are a key component of statistical inference and enable us to quickly explore a wide range of data. Simple and multiple linear regression model a continuous dependent variable as a function of one, and multiple, independent variables, respectively. 
- Linear models can be used for making predictions on new data, or doing inference on the coefficients of the model. In this book, we will be concerned with one of two different aspects: prediction or inference. We will use frequentist approaches for the former, and Bayesian methods for the latter. 
...
::: {.content-visible when-format="pdf"}
In this book we use frequentist GLMs within `tidymodels` when we are interested in prediction. And when we are focused on inference, instead, we use the probabilistic programming language Stan to fit GLMs in a Bayesian framework, and interface with it using `rstanarm` [@citerstanarm]. This is slightly simplistic---we could do prediction with Bayesian models, and we could use Stan within `tidymodels`. But, for the sake of simplicity and clarity, we keep these separated in this way for this book. It is important to be conversant in both ecosystems and cultures. These different cultures are also revealed in language choice. The primary language in this book is R, and for the sake of consistency we focus on that here. But there is an extensive culture, especially focused on prediction, that uses Python. We recommend focusing on only one language initially, but after developing that initial familiarity it is important to become multilingual and we introduce an approach based on Python in the ["Python" Online Appendix](https://tellingstorieswithdata.com/26-python.html).
:::

::: {.content-visible unless-format="pdf"}
In this book we use frequentist GLMs within `tidymodels` when we are interested in prediction. And when we are focused on inference, instead, we use the probabilistic programming language Stan to fit GLMs in a Bayesian framework, and interface with it using `rstanarm` [@citerstanarm]. This is slightly simplistic---we could do prediction with Bayesian models, and we could use Stan within `tidymodels`. But, for the sake of simplicity and clarity, we keep these separated in this way for this book. It is important to be conversant in both ecosystems and cultures. These different cultures are also revealed in language choice. The primary language in this book is R, and for the sake of consistency we focus on that here. But there is an extensive culture, especially focused on prediction, that uses Python. We recommend focusing on only one language initially, but after developing that initial familiarity it is important to become multilingual and we introduce an approach based on Python in @sec-python.
:::
...
### Inference with Bayesian methods {#sec-inferencewithbayesianmethods}
