...
We use `augment()` from `broom` to add the fitted values and residuals to our original dataset. This allows us to plot the residuals (@fig-fivekmvsmarathonresids).
...
::: {.content-visible when-format="pdf"}
In this book we focus on inference using the probabilistic programming language Stan\index{Stan} to fit models in a Bayesian\index{Bayesian} framework, and interface with it using `rstanarm`. Inference and forecasting have different cultures, especially when it is termed machine learning. It is important to be conversant in both ecosystems and cultures. These different cultures are also revealed in language choice. The primary language in this book is R, and for the sake of consistency we focus on that here. But there is an extensive culture, especially but not exclusively focused on prediction, that uses Python. We recommend focusing on only one language and approach initially, but after developing that initial familiarity it is important to become multilingual and we introduce an approach based on Python in the ["Prediction" Online Appendix](https://tellingstorieswithdata.com/26-prediction.html).
:::

::: {.content-visible unless-format="pdf"}
In this book we focus on inference using the probabilistic programming language Stan to fit models in a Bayesian framework, and interface with it using `rstanarm`. Inference and forecasting have different cultures, especially when it is termed machine learning. It is important to be conversant in both ecosystems and cultures. These different cultures are also revealed in language choice. The primary language in this book is R, and for the sake of consistency we focus on that here. But there is an extensive culture, especially but not exclusively focused on prediction, that uses Python. We recommend focusing on only one language and approach initially, but after developing that initial familiarity it is important to become multilingual and we introduce an approach based on Python in [Online Appendix -@sec-predictingpythons].
:::
