...
It may seem remarkable that he was considered an expert and his views had influence. Yet today, many will, say, use inflation statistics without ever having tried to track a few prices, use the responses from political surveys without themselves ever having asked a respondent a question, or use ImageNet\index{ImageNet} without the experience of hand-labeling some of the images. We must always throw ourselves into the details of the data.
...
The actual sum of the population is 5,050. We can obtain this using a trick attributed to Leonhard Euler,\index{Euler, Leonhard} the eighteenth-century mathematician, who noticed that the sum of one to any number can be quickly obtained by finding the middle number and then multiplying that by one plus the number. In this case, we have $50 \times 101$. Alternatively we can use R: `sum(1:100)`.
