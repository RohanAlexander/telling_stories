...
13. Based on @statcanhistory, why is the quality of census data evaluated (pick one)?
    a. To improve data privacy
    b. To reduce the cost of future censuses
    c.  To ensure census data are reliable and meet user needs
    d. To limit data dissemination
14. Based on @statcanhistory, what is the primary source of sampling error in the Census of Population (pick one)?
    a. Misclassification of dwellings.
    b. Non-response from individuals.
    c.  The use of a sample.
    d. Data capture errors during processing.
15. Based on @statcanhistory, which error occurs when people or dwellings are omitted or counted more than once (pick one)?
    a. Processing error
    b. Sampling error
    c.  Coverage error
    d. Non-response error
16. Based on @statcanhistory, which type of error is related to misunderstanding or misreporting by respondents or enumerators (pick one)?
    a. Sampling error
    b.  Response error
    c. Coverage error
    d. Processing error
17. Based on @statcanhistory, what is the purpose of the Dwelling Classification Survey (DCS) (pick one)?
    a.  To study classification errors in dwellings
    b. To collect data on household income
    c. To classify new housing developments
    d. To improve the long-form questionnaire
18. Based on @statcanhistory, what is the Census Undercoverage Study (CUS) designed to estimate (pick one)?
    a. Non-response rates for long-form questionnaires
    b.  The number of people omitted from the census
    c. The variance of sampling errors
    d. Imputation rates for missing data
19. Based on @statcanhistory, what does the Census Overcoverage Study (COS) identify (pick one)?
    a. People who were missed by the census
    b.  Cases where individuals were counted more than once
    c. Dwellings that were misclassified
    d. Invalid data entries during processing
20. Based on @statcanhistory, wow is the Total Non-Response (TNR) rate for the census defined (pick one)?
    a. The percentage of incorrect responses in the census.
    b.  The proportion of dwellings where the questionnaires does not meet the minimum content.
    c. The number of imputed values in the census data.
    d. The percentage of questionnaires with partial responses.
...
27. What is the main difference between "probability" and "non-probability" sampling (pick one)?
...
30. In which sampling method is the first unit selected at random, and subsequent units are selected at regular intervals (pick one)?
    a. Stratified sampling
    b. Cluster sampling
    c.  Systematic sampling
...
34. Please consider the integers [1:100]. If I were interested in implementing a sampling approach, based on a sample of only 10, to estimate the median, which approach would I choose?
...
36. As discussed in @bowley1913working, how was the sample for the study of Reading’s households selected (pick one)?
...
37. What is the sampling approach used by @bowley1913working (pick one)?
    ...
    c. Stratified sampling
    d.Cluster sampling
38. As discussed in @bowley1913working, which method was employed to provide estimates for the whole of Reading, based on the sample data (pick one)?
    a.  Applying a multiplier
    b. Calculating median income levels
    c. Using proportional census data
39. As discussed in @bowley1913working, what was the method of collecting rent and earnings data from the working-class households in Reading (pick one)?
...
41. Which of the following best describes convenience sampling (pick one)?
...
42. What is snowball sampling commonly used for (pick one)?
...
43. What is respondent-driven sampling (pick one)?
...
44. Based on @neyman1934two, what is the primary goal of stratified sampling (pick one)?
...
45. What was the main focus of @neyman1934two (pick one)?
...
46. Based on @neyman1934two, what is one advantage of stratified sampling over simple random sampling (pick one)?
    a. It requires fewer samples.
    b.  It may increase the precision of estimates.
    c. It eliminates the need for a sampling frame.
    d. It is cheaper.
47. Based on @neyman1934two [Section V], which approach allows a consistent estimate of the average collective character, X, of a population, whatever the properties of the population (pick one)?
    a.  Random sampling
