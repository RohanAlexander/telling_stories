...
- `canlang` [@canlang] (not on CRAN so install with: `devtools::install_github("ttimbers/canlang")`)
...
We can explore some data on languages spoken in Canada from the 2016 Census using `canlang`. This package is not on CRAN, but can be installed from GitHub with: `devtools::install_github("ttimbers/canlang")`).\index{packages!install}
