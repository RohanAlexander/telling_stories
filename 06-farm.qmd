...
- Read *Guide to the Census of Population*, [@statcanhistory]
  - Focus on Chapter 9 "Data quality evaluation". Despite being written by the Canadian government about a particular census, this chapter describes and addresses concerns that are relevant to almost any survey.
...
- Before there is a dataset, there must be measurement, and this brings many challenges and concerns. One dataset that is designed to be complete in certain respects is a census. While not perfect, governments spend much money on censuses and other official statistics, and they are a great foundational data source.
...
All datasets have shortcomings. In this chapter we develop comfort with "farmed data". We use that term to refer to a dataset that has been developed for the purpose of being used as data.

Even though these farmed datasets are put together for us to use, and can generally be obtained, it is nonetheless especially important for us to develop a thorough understanding of their construction. James Mill, the nineteenth century Scottish author, famously wrote *The History of British India* without having set foot in the country. He claimed: 
...
1. *(Plan)* Consider the following scenario: *Every day for a year two people---Mark and Lauren---record the amount of snow that fell that day in the two different states they are from.* Please sketch what that dataset could look like and then sketch a graph that you could build to show all observations.
...
1. What is a primary challenge when converting some phenomena in the world into a dataset (pick one)?
    a. The lack of available data collection tools.
    b.  Deciding what to measure and how to measure it appropriately.
    c. The overabundance of unbiased data.
    d. The high cost of data storage solutions.
...
3. According to metrology, which of the following best defines measurement (pick one)?
    a. The act of assigning numbers to objects arbitrarily.
    b.  The process of experimentally obtaining quantity values that can be attributed to a property of a phenomenon, body, or substance.
    c. The calculation of statistical significance in data analysis.
    d. The estimation of unknown variables using predictive models.
...
6. What does the "validity" of a measurement refer to (pick one)?
    a. The precision with which a measurement can be replicated.
    ...
    d. The statistical significance of the measurement results.
...
8. Which of the following best describes "measurement error" (pick one)?
    ...
    b. A deliberate alteration of data to mislead analysis.
    c. An error that occurs only when data is not normally distributed.
    d. A type of error that can be completely eliminated with better instruments.
9. What is censored data (pick one)?
    a. Data that have been intentionally omitted due to privacy concerns.
    b. Data collected from unauthorized sources.
    c. Data that have been corrupted and is unreadable.
    d.  Data where the value of an observation is only partially known.
10. How do truncated data differ from censored data (pick one)?
    a.  In truncated data, certain values are completely omitted from the dataset, whereas in censored data, the values are partially known but incomplete.
    c. Truncated data are less accurate than censored data.
    d. Truncated data deal with overestimation, censored data with underestimation.
11. What is missing completely at random (MCAR) (pick one)?
    a. Missing data where the likelihood of being missing is related to the unobserved data.
    b. Missing data where the likelihood of being missing is related to the observed data.
    c.  Missing data where the missingness is entirely random and unrelated to any data, observed or unobserved.
    d. Missing data that can be easily predicted using statistical models.
12. Why are censuses considered crucial datasets (pick one)?
    a.  They aim to collect data on every unit in a population, providing comprehensive datasets designed for analysis.
    b. They are conducted infrequently and thus have novel information.
    c. They focus exclusively on agricultural data, which is vital for economic planning.
    d. They are private datasets that offer exclusive insights.
13. With reference to @chen2019forensic and @martinez2019much, to what extent do you think we can trust government statistics? Please write at least a page and compare at least two governments in your answer.
...
15. Please use IPUMS to access the 2020 ACS. Making use of the codebook, how many respondents were there in California (STATEICP) that had a Doctoral degree as their highest educational attainment (EDUC) (pick one)?
    a.  4,684
    b. 5,765
    c. 2,007
    d. 732
16. Please use IPUMS to access the 1940 1% sample. Making use of the codebook, how many respondents were there in California (STATEICP) with 5+ years of college as their highest educational attainment (EDUC) (pick one)?
    a. 1,789
    b. 1,056
    c. 532
    d.  904
17. With reference to @Dean2022, please discuss the difference between probability and non-probability sampling.
18. What is a target population (pick one)?
    a. The list of all units in the target population from which a sample can be drawn.
    b. A subset of the population that is easily accessible for sampling.
    c.  The entire group about which the researcher wants to draw conclusions.
    d. The list of individuals who have agreed to participate in the study.
19. What is a sampling frame (pick one)?
    a.  The list of all units in the target population from which a sample can be drawn.
    b. The method used to collect data from respondents. 
...
20. What is the main difference between "probability" and "non-probability" sampling (pick one)?
    a. Probability sampling is always more cost-effective than non-probability sampling.
    b. Non-probability sampling methods are more accurate than probability sampling methods.
    c. Probability sampling does not require a sampling frame.
    d.  In probability sampling, every unit has a known chance of being selected, whereas in non-probability sampling, selection is not based on probabilities.
21. With reference to @beaumont2020probability, do you think that probability surveys will disappear, and why or why not (please write a paragraph or two)?
22. Which sampling method involves selecting units such that every observation in the sampling frame has an equal chance of being chosen (pick one)?
    a. Stratified sampling
    b. Cluster sampling
    c. Systematic sampling
    d.  Simple random sampling
23. In which sampling method is the first unit selected at random, and subsequent units are selected at regular intervals (pick one)?
    a. Stratified sampling
    b. Cluster sampling
    c.  Systematic sampling
    d. Convenience sampling
24. What characterizes stratified sampling (pick one)?
    ...
    d. Participants recruit other participants through their networks.
25. How are units selected in cluster sampling (pick one)?
    a. The population is divided into subgroups, and a random sample is taken from each subgroup.
    b. By selecting every nth unit from a list.
    c.  By randomly selecting entire groups or clusters, then sampling all or some units within them.
    d. By choosing units based on specific quotas.
26. Please name some reasons why you may wish to use cluster sampling (select all that apply)?
    a.  Balance in responses in terms of sub-populations.
    b.  Administrative convenience.
    c.  Efficiency in terms of money.
27. Please consider the integers [1:100]. If I were interested in implementing a sampling approach, based on a sample of only 10, to estimate the median, which approach would I choose?
    a. Simple random sampling
    b. Cluster sampling
    c. Stratified sampling
    d.  Systematic sampling
27. Write R code that considers the numbers 1 to 100, and estimates the mean, based on a cluster sample of 20 numbers. Re-run this code one hundred times, noting the estimate of the mean each time, and then plot the histogram. What do you notice about the graph? Add a paragraph of explanation and discussion.
28. As discussed in @bowley1913working, how was the sample for the study of Reading’s households selected (pick one)?
    a. By interviewing every fifth household
    b. By random selection of streets
    c.  By marking one building in ten from the local directory
    d. By selecting households based on income
29. What is the sampling approach used by @bowley1913working (pick one)?
    a. Simple random sampling
    b.  Systematic sampling
    c. Stratified sampling
    d.Cluster sampling
30. As discussed in @bowley1913working, which method was employed to provide estimates for the whole of Reading, based on the sample data (pick one)?
    a.  Applying a multiplier
    b. Calculating median income levels
    c. Using proportional census data
31. As discussed in @bowley1913working, what was the method of collecting rent and earnings data from the working-class households in Reading (pick one)?
    a. By interviewing landlords
    b. By inspecting census records
    c.  Through volunteer interviews with households
    d. By collecting data from tax records
...
33. Which of the following best describes convenience sampling (pick one)?
    a. Selecting participants based on random numbers.
    b. Using algorithms to select a sample.
    c. Sampling that ensures every subgroup is represented proportionally.
    d.  Choosing participants who are easiest to access.
34. What is snowball sampling commonly used for (pick one)?
    a.  Researching hidden or hard-to-reach populations by having existing participants recruit future participants.
    b. Estimating the total population size using capture-recapture methods.
    c. Studying well-defined populations with comprehensive sampling frames.
    d. Ensuring equal representation across different demographic groups.
35. What is respondent-driven sampling (pick one)?
    a. A type of random sampling where respondents are selected based on a probability mechanism.
    b. A sampling technique that uses automated systems to select respondents.
    c. A sampling method that relies on respondents volunteering without any recruitment efforts.
    d.  A form of non-probability sampling where respondents refer other respondents, often used for hidden populations, and includes incentives for recruitment.
1. Based on @neyman1934two, what is the primary goal of stratified sampling (pick one)?
    a. To increase bias in sample selection.
    b.  To ensure all strata of the population are represented.
    c. To reduce the need for randomization.
16. What was the main focus of @neyman1934two (pick one)?
    a. Introduction of the simple random sample.
    b. Elimination of all biases in sample selection.
    c. Development of quota sampling techniques.
    d.  The distinction between stratified sampling and non-probability sampling.
4. Based on @neyman1934two, what is one advantage of stratified sampling over simple random sampling (pick one)?
    a. It requires fewer samples.
    b.  It may increase the precision of estimates.
    c. It eliminates the need for a sampling frame.
    d. It is cheaper.
5. Based on @neyman1934two [Section V], which approach allows a consistent estimate of the average collective character, X, of a population, whatever the properties of the population (pick one)?
    a.  Random sampling
    b. Purposive sampling
36. Pretend that we have conducted a survey of everyone in Canada, where we asked for age, sex, and gender. Your friend claims that there is no need to worry about uncertainty "because we have the whole population". Is your friend right or wrong, and why?
37. With reference to @meng2018statistical, please discuss the claim: "When you have one million responses, you do not need to worry about randomization".
38. Imagine you take a job at a bank and they already have a dataset for you to use. What are some questions that you should explore when deciding whether that data will be useful to you?
...
Write a short (2ish pages + appendices + references)  paper using Quarto. Submit a link to a GitHub repo (one repo per group) that meets the general expectations of the course. Don't forget to cite the data (but don't upload the raw data to GitHub). Your paper should cover at least:
