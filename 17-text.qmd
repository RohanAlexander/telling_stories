...
We will now use `astrologer` [@astrologer], which is a dataset of horoscopes to explore a real dataset. This package is not on CRAN, so we need to install it.
...
There are four variables: "startdate", "zodiacsign", "horoscope", and "url". We are interested in the words used to distinguish the horoscope of each zodiac sign.
...
We can see that there are 106 horoscopes for each zodiac sign. In this example we will tokenize by word, and as we are interested in distinguishing between zodiac signs, we will concatenate each of those 106 to construct a corpus of 12 documents, one for each zodiac sign.
...
horoscope_corpus <- 
  horoscopes |> 
  group_by(zodiacsign) |> 
  summarise(all = paste0(horoscope, collapse = "; ")) |> 
  corpus(docid_field = "zodiacsign", 
         text_field = "all")
...
head(dfm_tfidf(horoscope_dfm))[,1:5]

library(quanteda.textstats)
sdf <- 
  textstat_frequency(horoscope_dfm,
                          groups = docs)
...
This will take some time, likely 10-30 min, so it is useful to save the model when it is done using `write_rds()`, and use `beep` to get a notification when it is done. We could then read the results back in with `read_rds()`.
