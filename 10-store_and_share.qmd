...
We can implement a differentially private approach using `diffpriv` [@diffpriv].
...
CSVs are commonly used without much thought in data science. And while CSVs are good because they have little overhead and can be manually inspected, this also means they are quite minimal. This can lead to issues, especially in terms of class, file size, and speed. There are various alternatives, including Apache Arrow, which is a columnar data framework.  There is an R implementation of Apache Arrow, `arrow` [@arrow]. This has the advantage of requiring very little change from us while delivering significant benefits. This makes two, distinct, file formats available to us: ".arrow", and ".parquet". We focus on parquet, which like CSV is an open standard, with many implementations.
...
At some point we might replace CSVs with parquet files. We can do this by replacing `write_csv()` with `write_parquet()`, and `read_csv()` with `read_parquet()`. While the decision to do this needs to consider both costs and benefits, having a schema attached in parquet makes dealing with, say, dates considerably easier in the short-term, and this is especially useful in the long-term.
