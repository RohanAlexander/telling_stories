...
For an example of differential privacy, consider a situation in which a professor wants to release a distribution of grades for a particular assignment. They are interested in ensuring that no student is able to work out the grade that another student got. For instance, let us consider a small class where there were 2 As, 3 Bs, and 2 Cs. Imagine if the professor announced this exact distribution, say "There were 2 As, 3 Bs, and 2 Cs". If all-but-one student eliminated their own grade from the public distribution, only the grade of the student that the other students are trying to find out about would be left. 

A non-statistical approach would be for the professor to add the word "roughly". So the professor would say "There were roughly 2 As, 3 Bs, and 2 Cs". The students could do the same elimination strategy, but they would never know with certainty. This probably satisfies the definition of differentially private, but if we wanted to use statistics then we could add some noise from a statistical distribution to the results. For instance, we could use `set.seed(853)` and then `runif(n = 3, min = 1, max = 3) |> round(0)` to generate `c(2 1 2)`. We could then add this to the actual results and report the following "There were 4 As, 4 Bs, and 4 Cs". We notice that this has not changed the median grade, which remains a B, but it has changed the mode, and would make the students' plan not inoperable, but less feasible. 
