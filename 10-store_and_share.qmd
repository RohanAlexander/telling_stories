...
CSVs are commonly used without much thought in data science. And while CSVs are good because they have little overhead and can be manually inspected, this also means they are quite minimal. This can lead to issues, especially in terms of class, file size, and speed. There are various alternatives, including Apache Arrow, which is a columnar data framework.  There is an R implementation of Apache Arrow, `arrow` [@arrow]. This has the advantage of requiring very little change from us while delivering significant benefits. 
<!-- This makes two, distinct, file formats available to us: ".arrow", and ".parquet".  -->
We focus on the ".parquet" format, which like CSV is an open standard, with many implementations.
...
In particular, we focus on the benefit of using parquet for data storage. Examples of this may include: 1) saving a copy of the original data; and 2) saving a copy of the analysis datasets. Parquet brings two particular benefits here, compared with CSV: 1) the file sizes are typically smaller; and 2) class is preserved.

Having loaded `arrow`, we can use parquet files in a similar way to CSV files. Anywhere in our code that we used `write_csv()` and `read_csv()` we could alternatively, or additionally, use `write_parquet()` and `read_parquet()`. While the decision to do this needs to consider both costs and benefits, having a schema attached in parquet makes dealing with, say, dates considerably easier.
...
We get significant reductions in file size when we compare the size of the same datasets saved in each format, especially as they get larger (@tbl-filesize).
...
@arrowcookbook provides further information about specific tasks, @navarro2021getting provides helpful examples of implementation, and @navarroworkshop provides an extensive set of materials.
