...
One thing to notice about that approach is that it would not work with persistent questioning. For instance, eventually the students would be able to back out the noise that was being added with some certainty. The result is that we need to limit, or "budget", the number of queries that we allow.
