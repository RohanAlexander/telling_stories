...
Code review is an important part of working as a professional [@codereview]. Please create a GitHub repository with a meaningful name, and appropriate structure, and put together a reproducible Quarto file that downloads a dataset using `opendatatoronto`, cleans it, and makes a graph. Then exchange it with someone else. Following the advice of @googlecoderview, please provide them with a review of their code by create an Issue. That should be at least two pages (an example is [here](https://github.com/RohanAlexander/telling_stories/issues/95)). You are marked on that peer review. Submit a link to the GitHub Issue.
