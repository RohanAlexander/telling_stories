...
5. *(Explore)* Assume that the `tidyverse` is loaded and the dataset "election_results" has the columns "seat", "party", and "votes", as in the earlier question. Which of the following would result in a count of the number of seats won by each party (pick one)? 
...
6. *(Communicate)* Please write two paragraphs as if you had gathered data from that source and built a graph. The exact details contained in the paragraphs do not have to be factual (i.e. you do not actually have to get the data nor create the graphs).
...
2. From @Gelman2016, what is the file drawer problem (pick one)?
...
3. From @Gelman2016, which term describes the multitude of choices researchers have in data analysis that can lead to significant results (pick one)?
    a.  Researcher degrees of freedom.
    b. Data mining.
    c. Sample bias.
    d. Effect size manipulation.
4. From @Gelman2016, the garden of forking paths refers to what issue in statistical analysis (pick one)?
    a. The complexity of decision trees in machine learning.
    b.  The multiple potential analyses paths that can be taken with the same data.
    c. The branching of psychological theories over time.
    d. The divergence of academic disciplines.
5. From @Gelman2016, what does the term replication crisis refer to (pick one)?
    a. Difficulty in creating new psychological theories.
    b. Overproduction of similar psychological research.
    c.  Challenges in replicating the findings of previous studies.
    d. Shortage of participants for experiments.
6. @Gelman2016 focuses on the replication crisis in psychology. Pick another discipline to focus on, based on your own experience, perhaps in other classes, and write about the extent to which you think there may be a replication in that discipline and why.
7. From @wilsongoodenough, which of the following is a key practice in data management (pick one)?
    a. Overwriting raw data files with cleaned-up versions.
    b. Avoiding documentation of data processing steps to save time.
    c. Using proprietary file formats for data storage.
    d.  Saving the raw data and ensuring it's backed up.
8. From @wilsongoodenough, why is it important to create a README file in the project's home directory (pick one)?
...
9. From @wilsongoodenough, what is a primary benefit of using a version control system in scientific computing projects (pick one)?
    a. It automatically writes code for the researcher.
    b.  It tracks changes over time and facilitates collaboration.
    c. It replaces the need for backing up data.
    d. It ensures that all data is encrypted.
10. From @wilsongoodenough, what is a recommended practice for naming files in a project (pick one)?
...
11. What is a benefit of reproducibility?
...
12. According to @Alexander2019 research is reproducible if (pick one)?
...
13. Which of the following statements best describes literate programming (pick one)?
...
14. What is the main function of git in a reproducible workflow (pick one)?
...
15. According to @tidyversestyleguide for naming files, how would the files "00_get_data.R" and "get data.R" be classified (pick one)?
    a. bad; bad.
    b.  good; bad.
    c. bad; good.
    d. good; good.
16. Which of the following would result in bold text in Quarto (pick one)?
    a.  `**bold**`
    b. `##bold##`
    c. `*bold*`
    d. `#bold#`
17. Which option would hide the warnings in a Quarto R chunk (pick one)?
    a. `echo: false`
    b. `eval: false`
    c.  `warning: false`
    d. `message: false`
18. Which option would run the R code chunk and display the results, but not show the code in a Quarto R chunk (pick one)?
    a.  `echo: false`
    b. `include: false`
    c. `eval: false`
    d. `warning: false`
    e. `message: false`
19. Why are R Projects important (select all that apply)?
    a.  They help with reproducibility.
    b.  They make it easier to share code.
    c.  They make your workspace more organized.
20. Assuming the packages and datasets have been loaded, what is the mistake in this code: `DoctorVisits |> filter(visits)` (pick one)?
    a. `visits`
    b. `DoctorVisits`
    c.  `filter`
    d. `|>`
21. What is a reprex and why is it important to be able to make one (select all that apply)?
...
22. According to @sharlatalks, what is the key part of "If you need help getting unstuck, the first step is to create a reprex, or reproducible example. The goal of a reprex is to package your problematic code in such a way that other people can run it and feel your pain. Then, hopefully, they can provide a solution and put you out of your misery." (pick one)?
...
23. In the context of reproducibility, what is the function of the `renv` package (pick one)?
    a. To run code chunks in parallel.
    b.  To document and share the software environment used for analysis.
    c. To automate code linting.
    d. To improve code efficiency in simulations.
24. Which of the following does NOT contribute to a reproducible workflow (pick one)?
    a.  Using `setwd()` to set the working directory.
    b. Sharing code and data along with research results.
    c. Using Quarto for integrating R code with reports.
    d. Using version control with Git and GitHub.
25. What is one advantage of parallel processing (pick one)?
...
Peer review generally, and code review specifically [@codereview], is an important part of working as a professional. 
...
Then exchange it with someone else. 

Following the advice of , please provide them with a review of their code by create an Issue. That should be at least two pages (an example is [here](https://github.com/RohanAlexander/telling_stories/issues/95)). You are marked on that peer review. Submit a link to the GitHub Issue.

Please read @googlecoderview and @giladpeerreview. Then using GitHub Issues please conduct peer review of the repo content. 

The peer review should have the following structure (based on @giladpeerreview): 

1. Opening statement summary 
    *[Add a brief summary of the manuscript you’re reviewing]*
2. List strong positive points
    *[Please keep this brief. Two or three dot points.]*
3. Critical improvements needed
    *[This is the most important section. These are issues that the authors of the paper must fix and/or address. It is important to keep those very constructive and polite, be gentle but clear, and provide as much information as you can to assist authors. This might include mistakes/errors, missing information, oversights, misunderstandings, etc. If you can, explain why these are mistakes with reference to where the correct information can be found.]*
4. Suggestions for improvement:
    *[This is you trying to help the authors do even better, as nice to have. You can comment on things that you are not sure of, or state an opinion, but be humble about this being a suggestion, and be very positive and constructive.]*
5. Please consider adding/changing/removing:
    *[Add five/six dot points around typos, minor code issues, etc]*
6. Evaluation: 
    *[If you had to grade this section based on the evaluation criteria provided in the syllabus and at the end, what would you currently give? This will =not= be used for grading, this is ONLY to provide authors with some idea of how much work they need to put into this section to improve.]*
7. Rubric:
    *[Add each element of the rubric and provide a mark against it.]*
8. Estimated overall mark
    *[X]* out of *[Y]*.
9. Reason:
    *[Add a brief sentence or two justifying the estimated mark.]*

Make sure that your review is nicely formatted.
