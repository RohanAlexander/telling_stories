...
5. According to @tidyversestyleguide for naming files, how would these files "00_get_data.R", "get data.R" be classified (pick one)?
    a. bad; bad.
    b.  good; bad.
    c. bad; good.
    d. good; good.
...
11. Consider this sequence: "`git pull`, `git status`, ________, `git status`, `git commit -m "My message"`, `git push`". What is the missing step (pick one)?
    ...
    c. `git pull`.
    d. `git push`.
12. Assuming the packages and datasets have been loaded, what is the mistake in this code: `DoctorVisits |> select("visits")` (pick one)?
    a.  `"visits"`
    b. `DoctorVisits`
    c. `select`
    d. `|>`
