...
4. From @Gelman2016, the garden of forking paths refers to what issue (pick one)?
...
5. From @Gelman2016, what does the replication crisis refer to (pick one)?
...
6. @Gelman2016 focuses on the replication crisis in psychology. Pick another discipline to focus on, based on your own experience, perhaps in other classes, and write about the extent to which you think there may be replication issues in that discipline and why.
7. From @wilsongoodenough, which of the following are important data management practices (select all that apply)?
...
9. From @wilsongoodenough, what is a primary benefit of using version control (pick one)?
...
13. What is literate programming (pick one)?
...
15. According to @tidyversestyleguide, how would the files "00_get_data.R" and "get data.R" be classified (pick one)?
    a. bad; bad.
    b.  good; bad.
    c. bad; good.
    d. good; good.
...
20. Why is it important that your R Project name reflects the content of your repo (select all that apply)?
    a.  Consistency.
    b.  Professionalism.
    c.  Attention to detail.
21. Assume the packages and datasets have been loaded; what is the mistake in this code: `DoctorVisits |> filter(visits)` (pick one)?
    a. `DoctorVisits`
    b. `|>`
    c.  `filter`
    d. `visits`
22. What is a reprex and why is it important to be able to make one (select all that apply)?
...
23. According to @sharlatalks, what is the key part of "If you need help getting unstuck, the first step is to create a reprex, or reproducible example. The goal of a reprex is to package your problematic code in such a way that other people can run it and feel your pain. Then, hopefully, they can provide a solution and put you out of your misery." (pick one)?
...
24. In the context of reproducibility, what is the function of the `renv` package (pick one)?
...
25. Which of the following does NOT contribute to a reproducible workflow (pick one)?
...
The purpose of this activity is to give and review peer review. Peer review generally, and code review specifically [@codereview], is an important part of working as a professional. 
