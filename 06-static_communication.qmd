...
- Read *R for Data Science*, Chapter 33 "Graphics for communication", [@r4ds]
...
beps |>
  ...
  labs(x = "Age-group of respondent",
       y = "Number of respondents",
       fill = "Voted for") +
  facet_wrap(vars(political.knowledge),
             dir = "v",
             scales = "free",
...
There are a variety of different ways to change the colors, and many palettes are available from `RColorBrewer` [@RColorBrewer], which we specify with `scale_fill_brewer()`, and `viridis` [@viridis], which we specify with `scale_fill_viridis()` and is particularly focused on color-blind palettes (@fig-usecolor). 
...
beta_distributions <-
  tibble(
    ...
    source = c(rep("Left and right skew", number_of_draws),
...
world_bank_data |>
  ...
  kable(
    col.names = c("Country", "Year", "Inflation", 
                  "GDP growth", "Population", "Unemployment rate"),
    digits = 1,
    booktabs = TRUE,
    linesep = ""
  )
...
We can use `kableExtra` [@citekableextra] to add extra functionality to `kable`. For instance, we could add a row that groups some of the columns using `add_header_above()` (@tbl-gdpalign). This impacts the spacing of the table, so we can also specify `kable_styling(full_width = TRUE)` to make sure that the table still fills the full width of our document.
