...
4.  *(Acquire)* Please specify one possible source of actual data about the amount of money that is donated to charity in a country that you are interested in.
...
1. What does @citeBarrett emphasize about learning data science (pick one)?
    ...
    d. Learning through failures.
2. How does @chambliss1989mundanity define excellence (pick one)?
...
3. Based on @chambliss1989mundanity, what is the key factor that leads to excellence (pick one)?
    a. Natural talent.
    b. Access to resources.
    c.  Discipline, technique, and attitude.
    d. Special training methods.
...
7. What is the first sentence in the help file for the `tidyverse` (hint: run `?tidyverse` in the console) (pick one)?
    a.  "The 'tidyverse' is a set of packages that work in harmony because they share common data representations and 'API' design."
    b. "Welcome to the 'tidyverse'."
    c. "Once there were four children whose names were Peter, Susan, Edmund, and Lucy."
8. Use a help file to determine which of the following are arguments for `read_csv()` (select all that apply)?
    a. "all_cols"
    b.  "file"
    c.  "show_col_types"
    d.  "number"
9. In the workflow advocated in *Telling Stories with Data*, what is the first step of the data science project process (pick one)?
    a. Explore.
    b. Simulate.
    c.  Plan.
    d. Share.
10. Which `R` package, within the core `tidyverse`, is mainly used for data manipulation (pick one)?
    ...
    c.  `dplyr`
    d. `lubridate`
11. Which function is used to improve column names to make them easier to work with (pick one)?
    a. `clean_names()`
    b. `mutate()`
    c. `rename()`
    d. `filter()`
...
16. In the workflow advocated in *Telling Stories with Data*, why do we simulate data (select all that apply)?
    ...
    c.  It helps with working in teams.
17. When using the `sample()` function in R, what does setting "replace = TRUE" do (pick one)?
    a. The same values cannot be repeated.
    b. Each value is unique.
    c.  The same value can be selected more than once.
    d. Values are replaced after they are used.
...
28. I run this code `library(datasauRus)` and get this error "Error in library(datasauRus) : there is no package called 'datasauRus'". What is the most likely issue (pick one)?
    a. Typo in the name `datasauRus`.
    b.  The ``datasauRus` package was not installed.
...
In this exercise we redo the Australian Elections example, but for Canada. By way of background, Canada's parliament has 338 seats, also known as "ridings", in the House of Commons. The main political parties are: major parties: Liberal and Conservative; minor parties: Bloc Québécois, New Democratic, and Green; some smaller parties and independents.
...
4. Explore: Make a nice graph of the number of ridings won by each party in the 2021 Canadian Federal Election.
5. Share: Write a few paragraphs about what you did, why you did it, and what you found.
