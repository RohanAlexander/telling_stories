...
5. What is the first sentence in the help file for the `tidyverse` that you get by running `?tidyverse`?
6.  Which of the following are arguments for `read_csv()` from `readr` (select all that apply)? (Hint: You can access the help for the function with `?read_csv()`.) a."all_cols"
    b.  "file"
    c.  "show_col_types"
    d.  "number"
7.  We used `rpois()` and `runif()` to draw from the Poisson and Uniform distributions, respectively. Which of the following can be used to draw from the Normal and Binomial distributions (select all that apply)?
    ...
    d.  `rnorm()` and `rbinom()`
8.  We used `rpois()` and `runif()` to draw from the Poisson and Uniform distributions, respectively. Which of the following can be used to draw from the Exponential and Cauchy distributions (pick one)?
    a.  `rexp()` and `rcau()`
...
12. Which function is used to update packages (pick one)?
    a.  `update.packages()`
    b.  `upgrade.packages()`
    c.  `revise.packages()`
    d.  `renovate.packages()`
13. What are some features that we might typically expect of a column that claimed to be a year (select all that apply)?
    a.  The class is "character".
    b.  There are no negative numbers.
    c.  There are letters in the column.
    d.  Each entry has four digits.
14. Please consider the following code, add a small mistake to it, and then create a GitHub Gist that contains all of the code, and submit the URL.
...
midwest |>
  ...
  scale_y_log10()
```
15. Why do we simulate a dataset (write at least three dot points)?
