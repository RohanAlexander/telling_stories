...
To get started, go to R Studio Cloud -- https://rstudio.cloud/ -- and create an account. As we are not doing anything too involved the free version will be fine for now. Once you have an account and log in, then it should look something like @fig-opening-r-studio-cloud.

![Opening R Studio Cloud for the first time](figures/opening-r-studio-cloud-for-the-first-time.png){#fig-opening-r-studio-cloud width=85% fig-align="center"}
...
At a certain point, your code will not run and you will want to ask others for help. We will discuss how to do this in Chapter \@ref(reproducible-workflows), but for now, if you need help, then you should create a GitHub Gist. The first step is to create an account on GitHub: https://github.com (@fig-githubone).
...
We are interested to make a graph with year on the x-axis and estimated NMR on the y-axis. Each country should have its own series Roughly similar to @fig-nmrexamplegraph
...
We can now make the graph that we are interested in (@fig-nmrgraph). We are interested in showing how NMR has changed over time and the difference between countries.
