...
- Read *Data science as an atomic habit*, [@citeBarrett]
  - *One way to approach learning data science.*
...
3. *(Acquire)* Please identify one possible source of actual data about the amount of money that is donated to charity in a country that you are interested in.
4. *(Explore)* Assume that `ggplot2` is loaded and the dataset "donations" has the column "amount". Which of the following would result in a bar chart (pick one)?
...
13. We used `rpois()` and `runif()` to draw from the Poisson and Uniform distributions, respectively. Which of the following can be used to draw from the Normal and Binomial distributions (select all that apply)?
    ...
    d.  `rnorm()` and `rbinom()`
14. We used `rpois()` and `runif()` to draw from the Poisson and Uniform distributions, respectively. Which of the following can be used to draw from the Exponential and Cauchy distributions (select all that apply)?
    a. `rexp()` and `rcau()`
    b. `rexpon()` and `rcauchy()`
    c.  `rexp()` and `rcauchy()`
    d. `rexpon()` and `rcau()`
15. What is the result of `sample(x = letters, size = 2)` when the seed is set to "853"? What about when the seed is set to "1234" (pick one)?
    a.  '"i" "q"' and '"p" "v"'
    b. '"e" "l"' and '"e" "r"'
    c. '"i" "q"' and '"e" "r"'
    d. '"e" "l"' and '"p" "v"'
16. Which function provides the recommended citation to cite R (pick one)? 
    a. `cite("R")`. 
    b. `cite()`.
    c. `citation("R")`.
    d. `citation()`.
17. How do we get the citation information for `opendatatoronto` (pick one)?
    a. cite()
    b. citation()
    c. cite("opendatatoronto")
    d. citation("opendatatoronto")
18. Which argument needs to be changed to change the headings in `kable()` from `knitr` [@citeknitr] (pick one)?
    a. "booktabs"
    b. "col.names"
    c. "digits"
    d. "linesep"
    e. "caption"
19. Which function is used to update packages (pick one)? 
    a. `update.packages()`
    b. `upgrade.packages()`
    c. `revise.packages()`
    d. `renovate.packages()`
20. What are some features that we might typically expect of a column that claimed to be a year (select all that apply)? 
    a. The class is "character".
    b. There are no negative numbers.
    c. There are letters in the column.
    d. Each entry has four digits.
21. Please consider @hao2019 and demonstrate a correspondence between the workflow steps and the stages that bias can enter as discussed by @hao2019. Then extend @hao2019 somehow to ensure it is clear that this is not a problem isolated to AI (for instance you may like to link it to @kennedy2020using).
21. Please consider the following code, add a small mistake to it, and then create a GitHub Gist that contains all of the code, and submit the URL.
