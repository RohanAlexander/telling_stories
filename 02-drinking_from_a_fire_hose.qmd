...
Within Posit Cloud, make a new Quarto document and save it. Add preamble documentation and set up the workspace. We will use the `tidyverse` [@Wickham2017], `janitor` [@janitor], and `lubridate` [@GrolemundWickham2011].
...
While this simulation works, it would be time consuming and error prone if we decided that instead of 50 years, we were interested in simulating, say, 60 years. One way to make this easier is to replace all instances of 50 with a variable.
