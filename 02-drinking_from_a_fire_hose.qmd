...
7. What is the first sentence in the help file for the `tidyverse`?
...
9. In the workflow advocated in *Telling Stories with Data*, what is the first step of the data science project process (pick one)?
    ...
    c.  Plan
    d. Share
10. Which `R` package is primarily used for data manipulation and is part of the core `tidyverse` (pick one)?
    a. `ggplot2`
    b. `janitor`
    c.  `dplyr`
    d. `lubridate`
11. Which function is used to improve column names to make them easier to work with (pick one)?
    ...
    c. `rename()`
    d. `filter()`
12. What is the main purpose of the `ggplot2` package (pick one)?
...
15. Why is it important to use `set.seed()` during simulation (pick one)?
    ...
    b.  To ensure reproducibility of random results
...
19. What distribution does `runif()` sample from (pick one)?
    ...
    c. Poisson distribution
    d. Exponential distribution
20. Which of the following can be used to draw from the Normal and Binomial distributions (select all that apply)?
